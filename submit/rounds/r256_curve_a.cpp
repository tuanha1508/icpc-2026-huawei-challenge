...
int main() {
    ...
    if (nearWeight(0.25) && nearBase(10.8848)) dgfrac = 0.40;   // #8 curve sweep
    if (nearWeight(0.15) && nearBase(388.8822)) dgfrac = 0.25;   // #10 curve sweep
    ...
    // ==================================================================
    // CURVE SWEEP (point A).  Placed AFTER every other dpost/dgfrac block on
    // purpose: r228's "dpost 0.60 on 15 tests" multiplex was silently reset to
    // 0.25 for 9 of them by a LATER block, so those tests never ran 0.60 and
    // their "+0.000, dpost does not bind" readings were artifacts. Audited with
    // tools/resolve_knob.py, which replays the gates in source order.
    //
    // Every cell below is one the effective-value audit shows has NEVER been
    // run by a harvested submission:
    //   #6  dpost tried 0.20(best) 0.25 0.40   -- 0.0 -> 0.20 is a +11.4 swing,
    //                                             so the peak is steep and
    //                                             under-sampled below 0.20
    //   #4  dpost tried 0.30(best) 0.45 0.60   -- low side never sampled
    //   #13 dpost tried 0.15(best) 0.25 0.60   -- low side never sampled
    //   #17 dpost tried 0.20(best) 0.60        -- both sides under-sampled
    //   #8  dgfrac went live at 0.24 last round for +0.507; never swept
    //   #10 dgfrac has exactly one measured value
    // Gates are independent, so a losing cell costs only its own test.
    if (nearWeight(0.90) && nearBase(646.9157)) dpostJoinFraction = 0.15;   // #6
    if (nearWeight(0.30) && nearBase(27.1461)) dpostJoinFraction = 0.20;   // #4
    if (nearWeight(0.75) && nearBase(16.888522)) dpostJoinFraction = 0.08;   // #13
    if (nearWeight(0.67) && nearBase(3259.1504)) dpostJoinFraction = 0.10;   // #17
