...
int main() {
    ...
    // Remote-load balance weight. The old 4.0 came from robust-72, whose six
    // weight groups all appear in the feedback set, so it proxies re-weighted
    // feedback bases rather than unseen work. Re-swept on 150 workloads at
    // weights the feedback set does NOT contain:
    //   0.25 +18.420  0.5 +24.485  0.75 +18.869  1.0 +23.707
    //   1.25 +31.792  1.5 +31.655  2.0 +11.243   4.0  0 (base)
    // Seven consecutive positives, no sign flip -- a real plateau, unlike the
    // single-point spikes that sank the dgfrac and dpostfrac candidates. 1.25
    // is the max and central, clear of the cliff below 0.25 (balw 0 = -2539).
