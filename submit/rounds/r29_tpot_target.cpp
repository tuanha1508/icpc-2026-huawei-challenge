...
int main() {
    ...
    // GLOBAL DEFAULT: the adaptive controller's TPOT target. At factor 1.0 it
    // begins shrinking Ntarget the instant tpot exceeds SLO2; at 1.2 it tolerates
    // 20% over first. The controller was over-throttling -- the same
    // too-conservative bias that rprio 'D' had, and that was worth +82.50.
    // Measured: unseen +83.1, preliminary +251.36. Positive on both, and the
    // rejected alternatives show why both matter: GROW=8 is +109.9 unseen but
    // -21.27 preliminary, TUNE=32 is +58.0 unseen but -369.93 preliminary.
    double tpotTarget = SLO2 * (dist_base == 0.0 ? 0.90 : 1.20);
