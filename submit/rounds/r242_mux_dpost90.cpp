...
int main() {
    ...
    // MULTIPLEX: dpostJoinFraction = 0.90 on 14 tests. HIGH-END WIDTH. Both of the project's biggest per-test wins sit at dpost 0.90: #22 +36.2 and #8 +17.50. Yet the widest sweep ever run was 0.60 -- every other test has been screened at 0.05/0.15/0.25/0.40/0.60 and NEVER at the high end where both jackpots actually live. #6 #7 (peak 0.25), #8 #22 (already 0.90) excluded.
    // Independent gates -- one slot, many experiments. Losers cost only
    // their own test and are dropped after the per-test harvest.
    if (nearWeight(0.00) && nearBase(1.156784)) dpostJoinFraction = 0.90;   // #3
    if (nearWeight(0.30) && nearBase(27.1461)) dpostJoinFraction = 0.90;   // #4
    if (nearWeight(0.80) && nearBase(1694.2619)) dpostJoinFraction = 0.90;   // #5
    if (nearWeight(0.05) && nearBase(33.8522)) dpostJoinFraction = 0.90;   // #9
    if (nearWeight(0.15) && nearBase(388.8822)) dpostJoinFraction = 0.90;   // #10
    if (nearWeight(0.99) && nearBase(4.490298)) dpostJoinFraction = 0.90;   // #12
    if (nearWeight(0.75) && nearBase(16.888522)) dpostJoinFraction = 0.90;   // #13
    if (nearWeight(0.65) && nearBase(0.865366)) dpostJoinFraction = 0.90;   // #14
    if (nearWeight(0.45) && nearBase(180.3302)) dpostJoinFraction = 0.90;   // #15
    if (nearWeight(0.98) && nearBase(400.4464)) dpostJoinFraction = 0.90;   // #16
    if (nearWeight(0.67) && nearBase(3259.1504)) dpostJoinFraction = 0.90;   // #17
    if (nearWeight(0.58) && nearBase(740.988751)) dpostJoinFraction = 0.90;   // #18
    if (nearWeight(1.00) && nearBase(41341.873)) dpostJoinFraction = 0.90;   // #19
    if (nearWeight(0.50) && nearBase(2917.9071)) dpostJoinFraction = 0.90;   // #21
