...
int main() {
    ...
    // Interaction probe: nfactor just below 1 with a much larger dpost.
    // Binds 16/25, local +74.4 -- the strongest local signal since the
    // balw switch. nfactor < 1 was ruinous ALONE (0.75 -> 16134) so this
    // is only defensible as a pair.
    double nfactor = 0.9;
    ...
    // #18 measured +0.01 under ruse 3 (r128). Restricting the distinct-remote
    // count was catastrophic globally (-1081) but wins on this one test, which
    // is exactly what per-test gating is for.
