...
int main() {
    ...
    // 'S' is shortest-job-first, which minimises MEAN flow time and is the
    // right policy if mean TDR were the whole objective. But dist is
    // sqrt(ex_tdr^2 + ex_tpot^2) with each excess clamped at zero, so it is
    // nonlinear in the tail rather than linear in the mean: a few badly
    // starved long requests can dominate it in a way SJF does not price.
    // The third branch, key = (t - arrivalT) / svcEst, is a
    // highest-response-ratio rule that ages waiting requests and is never
    // reachable by default. Any letter that is not F/L/S selects it.
    char order = (w_c > 0.0) ? 'R' : 'F';
