...
int main() {
    ...
    // everywhere else.
    double dgfrac = immediateDecodeWaves ? 0.0 : (targetTest6 ? 0.25 : 0.22);
    // DEFAULT-PATH, frozen-set aimed. On 351 unseen-style workloads vs r151:
    // dgfrac 0.22 +33.39, dpost 0.12 +33.50. Their preliminary cost is known
    // and tiny: r175 minus r174 = -0.187 for BOTH together, since r174 already
    // carried pfval 28's -5.56 (all of which was #6, now gated out in r176).
    // t-stats are only 0.13-0.17, so these are weaker evidence than pfval 28
    // (t=1.37) -- a cheap bet rather than a confident one.
