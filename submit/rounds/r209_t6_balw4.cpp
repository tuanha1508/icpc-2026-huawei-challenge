...
int main() {
    ...
    // #6, rebased on r204 (16319.510, current best). dpost is now fully mapped
    // and closed: 0.15 -11.6 | **0.25 BEST** | 0.32 -0.70 | 0.40 -8.7 |
    // 0.55 -100 | 0.90 -35. Sharply peaked at 0.25.
    // Component: balw. The global default is the COUNT-balancing branch (-1),
    // which won overall at r104. Only #17 and #13 have ever been gated back to
    // the work-weighted branch. #6 has never been tried on it, and remote
    // selection governs the serial blocking that limits its throughput.
    if (nearWeight(0.90) && nearBase(646.9157)) balw = 4.0;   // #6
