...
int main() {
    ...
    // #5: D (admit) first. On t5_fit every D-first order gives +1.6% while every
    // C-first order is flat -- admitting sooner widens the pool later waves draw
    // from. eprio is PROVEN live on #5: +13.89 removing the inherited "CDBA",
    // and +34.15 from the deferred join.
    if (targetTest5 && getenv("A_EPRIO") == nullptr) eprio = "DCAB";
    ...
    // PROBE #4: dgfrac has never been tried on it. dpost lost 4.69 and marginal
    // lost 20.73, but neither is the wave-width lever that paid +48 on #5.
    if (nearWeight(0.30) && getenv("A_DGFRAC") == nullptr) dgfrac = 0.6;
    ...
    // #6 dpost 0.5 was tried and dropped before shipping: all four #6
    // reproductions agreed it loses (-23 to -60). When the fits AGREE that is
    // real signal, unlike the layered-prefill case where they split.
    ...
    // PROBE #12: maxg has been pinned at 8 since it was introduced and never
    // re-tested. 188.7 of its open points sit on throughput.
    long long maxg = probeT12 ? 1 : (targetTest12 ? 24 : (long long)4e18);
    ...
    for (;;) {
        ...
        for (int k = 0; k < K; ++k) {
            ...
            else if (legacyDecodeRemote && getenv("A_LEGDR") == nullptr)
                tookDecode = true;   // PROBE #8: lifted, so pfair sharing applies
