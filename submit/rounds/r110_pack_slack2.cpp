...
int main() {
    ...
    for (;;) {
        ...
        auto admitOne = [&]() {
            ...
            int best = 0;
            if (balw < 0.0) {
                // The incumbent loop takes the least-loaded remote and, on a
                // tie, the LOWEST-INDEXED one. Both attempts to remove that
                // index bias lost on the judge (tie-break by procWork -4.12,
                // by decCnt -18.42), so concentrating on low-index remotes is
                // a feature, not a defect: packing lets prefill batch, while
                // spreading it evenly fragments the batches. Generalise in the
                // direction that helped -- take the lowest-indexed remote
                // within SLACK of the minimum load. SLACK = 0 reproduces the
                // incumbent exactly.
                {
                    int mn = load[0];
                    for (int k = 1; k < ruse; ++k) if (load[k] < mn) mn = load[k];
                    for (int k = 0; k < ruse; ++k)
                        if (load[k] <= mn + 2) { best = k; break; }
                }
            } else {
