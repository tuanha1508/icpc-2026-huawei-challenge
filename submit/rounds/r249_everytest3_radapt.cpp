...
int main() {
    ...
    // radapt has never been varied per-test except on #6 (no-op) and #8 (-1.50).
    // The adaptive-concurrency loop steers N on dist; on the throughput-heavy
    // tests dist is the minor term, so disabling it may free the schedule.
    {
