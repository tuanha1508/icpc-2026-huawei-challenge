...
int main() {
    ...
    bool useMarginal = !((nearWeight(0.05) && !nearBase(33.8522)) ||  // #9 class
                         ...
                         legacyQuarter);
    // EVERY-TEST PROBE #4: useMarginal per test. The greedy rate-maximiser has
    // only ever been switched off for #6 (-19.7), #8, #9, #10, #13 and #15.
    // These eleven have never been tried without it -- and #9's exclusion was
    // worth +0.041 when it went the OTHER way, so the axis does carry signal.
    {
