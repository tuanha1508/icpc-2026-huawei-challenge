...
int main() {
    ...
    // GLOBAL DEFAULT: E-side order. C = P POST, D = P PRE (prefill);
    // A = D POST, B = D PRE (decode). "CDAB" -> "DCBA" moves D PRE ahead of
    // P POST and pushes D POST last. Measured: unseen +90.2, preliminary +1.52
    // -- positive on both, which is the most that can be said for any remaining
    // candidate. eprio is a demonstrably LIVE lever on the judge (it was worth
    // +13.89 on #5 and +3.24 on #13 when gated), unlike the adaptive-controller
    // constants which measured exactly 0.00.
    string eprio = "DCBA";
    ...
    for (;;) {
        ...
        if (legacyQuarter && w_c > 0.0 && nfactor > 0.0 &&
            ...
            if (!legacyDecodeFirst && !eprioForced && w_c >= wTpEff) {
                eprio = (exTpot > exTdr) ? EPRIO_DECODE : "DCBA";
            }
