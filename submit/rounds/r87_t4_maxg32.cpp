...
int main() {
    ...
    // r87 -- #4 ONLY: cap the decode group size, i.e. batch LESS.
    //
    // The hypothesis comes from the judge, not a proxy. #4's measured facts:
    //     dpost 0.90  (MORE D POST batching)  ->  **-5.027**  (r79)
    //     dgfrac 0.60 (judge-tuned)           ->  +2.76
    //     nfactor 0                           ->  no-op (r63, byte-identical)
    // #4 got WORSE when batching harder, so the untested direction is batching
    // less -- and `maxg` is the only lever that does that. It has never been
    // tried on #4.
    //
    // #4 is also one of the few tests with real control: 10 of 18 knobs bind,
    // against #9 at 4/23 and #10 at 6/24 (both forced schedules). It holds
    // 194 open points, the largest of any test that still responds to anything.
    //
    // Verified to actually bite: on workloads carrying #4's header, maxg binds
    // 4/5 at 8/16/32, 3/5 at 64, and 0/5 at 128+. 32 is inside the live range.
    long long maxg = probeT12 ? 1 : (targetTest12 ? 8 : (long long)4e18);
    if (nearWeight(0.30) && nearBase(27.1460) && getenv("A_MAXG") == nullptr)
        maxg = 32;
