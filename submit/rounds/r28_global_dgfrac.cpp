...
int main() {
    ...
    // GLOBAL DEFAULT: base decode-wave wait 0.25 -> 0.10, chosen against unseen
    // workloads rather than the feedback set, and confirmed on both:
    //     72 unseen-style workloads : +135.7
    //     58 preliminary-style tests: +63.94
    // 0.0 is close behind (+128.4 / +45.60); 0.10 wins on both so it takes it.
    // Un-gated, so it reaches all 20 scored tests. The weight-gated overrides
    // below (w 0.30 -> 0.60, the #16 flat-curve 0.95) still apply on top.
    double dgfrac = immediateDecodeWaves ? 0.0 : 0.10;
