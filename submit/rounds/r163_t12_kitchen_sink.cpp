...
int main() {
    ...
    // KITCHEN-SINK probe for #12: every INDEPENDENT lever at once. If this
    // beats r151, bisect; if it comes back flat, the whole branch dies in one
    // submission instead of four. Conflicting variants (eprio ABCD vs BACD,
    // order R vs K, dgfrac 0.60 vs 0.70) cannot be bundled and stay separate.
    char order = (fabs(w_tp - 0.99) < 1e-9 && fabs(dist_base / 4.490298 - 1.0) < 1e-3) ? 'K' : ((w_c > 0.0) ? 'S' : 'F');
    ...
    bool radapt = !targetTest5 && !legacyQuarter && (getenv("A_RUSE") == nullptr)
                  && !(nearWeight(0.99) && nearBase(4.490298));
