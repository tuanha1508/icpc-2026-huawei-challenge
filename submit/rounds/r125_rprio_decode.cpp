...
int main() {
    ...
    // Remotes sit idle 94.6% of the time and essentially never because work is
    // queued behind a pinned request (head-of-line blocking measured at 0.0%),
    // so the binding resource is the LINK, not remote compute. Each prefill UP
    // transfer carries L_in tokens -- up to 4096 -- and sits in the FIFO ahead
    // of decode transfers. rprio decides what a free remote runs, which is what
    // feeds that FIFO. The prefill-first default was set in r27 on a much older
    // build, before the balw switch and the current gate set; re-measure it.
    // #6 and #10 already override to 'D' by gate and both like it.
    char rprio = 'D';
