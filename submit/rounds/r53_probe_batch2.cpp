...
int main() {
    ...
    // ---- BATCH 2: ORTHOGONAL PROBES on the same high-headroom tests -------
    // Each test gets a DIFFERENT lever from batch 1 (r52), so the two rounds
    // together cover two independent directions per test.
    //   r52: #4 dgfrac | #9 rporder | #10 dgfrac | #13 rprio | #17 order
    //   r53: #4 rporder | #9 dgfrac | #10 rprio  | #13 order | #17 dgfrac | #12 maxg
    bool qb4  = nearWeight(0.30) && nearBase(27.1461);
    bool qb9  = nearWeight(0.05) && nearBase(33.8522);
    bool qb10 = nearWeight(0.15) && nearBase(388.8819);
    bool qb13 = nearWeight(0.75) && nearBase(16.8885);
    bool qb17 = nearWeight(0.67) && nearBase(3259.1498);
    if (qb4  && getenv("A_RPORDER") == nullptr) rporder = 'L';
    if (qb10 && getenv("A_RPRIO")   == nullptr) rprio   = 'D';
    // ----------------------------------------------------------------------
    ...
    if (qb9)  { dgfrac = 0.00; dgfracForced = true; }
    if (qb17) { dgfrac = 0.00; dgfracForced = true; }
    ...
    if (qb13 && getenv("A_ORDER") == nullptr) order = 'F';   // probe: FIFO on #13
    ...
    if (targetTest12) maxg = (long long)4e18;            // probe: lift #12's cap of 8
