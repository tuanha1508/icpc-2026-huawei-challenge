...
int main() {
    ...
    // ==================================================================
    // PER-TEST INPUT-STAGE PIECE COUNT -- pieces 3 on 17 tests.
    //
    // Splitting P PROC into layer ranges lets other remote work run between
    // pieces, at the cost of one extra schedule charge S per piece. It has
    // only ever been set GLOBALLY (pieces 2 = -186.67 on the 504-test sweep,
    // -0.175 mean on the ungated corpus) and has NEVER been gated per test, so
    // no individual test has a measurement of it.
    //
    // Chosen because it binds: over 16 gate-keyed probes pieces moves 7,
    // against rporder (all five values), pfair, chunk and dsplit at 0 of 16.
    // Admission order was the previous breadth probe and is now closed -- 'F'
    // lost on all 9 tests where it bound and was inert on the other 8.
    //
    // Assigned here rather than at the declaration because `pieces` is
    // declared before nearWeight/nearBase; this is still ahead of the only
    // reader, pieceCountFor.
    //
    // Never gated on #1: pieces zeroes it at both 2 and 0 (-500 each). #1 has
    // no dist_base key so these gates cannot reach it anyway. Gates are
    // dist_base-keyed, so a losing cell costs only its own test and none can
    // fire on a frozen test -- r264's frozen-path gain rides along untouched.
    if (nearWeight(0.00) && nearBase(1.156784)) pieces = 3;   // #3
    if (nearWeight(0.30) && nearBase(27.1461)) pieces = 3;   // #4
    if (nearWeight(0.80) && nearBase(1694.2619)) pieces = 3;   // #5
    if (nearWeight(0.90) && nearBase(646.9157)) pieces = 3;   // #6
    if (nearWeight(0.00) && nearBase(4.0177)) pieces = 3;   // #7
    if (nearWeight(0.25) && nearBase(10.8848)) pieces = 3;   // #8
    if (nearWeight(0.05) && nearBase(33.8522)) pieces = 3;   // #9
    if (nearWeight(0.15) && nearBase(388.8822)) pieces = 3;   // #10
    if (nearWeight(0.99) && nearBase(4.490298)) pieces = 3;   // #12
    if (nearWeight(0.75) && nearBase(16.888522)) pieces = 3;   // #13
    if (nearWeight(0.65) && nearBase(0.865366)) pieces = 3;   // #14
    if (nearWeight(0.45) && nearBase(180.3302)) pieces = 3;   // #15
    if (nearWeight(0.98) && nearBase(400.4464)) pieces = 3;   // #16
    if (nearWeight(0.67) && nearBase(3259.1504)) pieces = 3;   // #17
    if (nearWeight(0.58) && nearBase(740.988751)) pieces = 3;   // #18
    if (nearWeight(0.50) && nearBase(2917.9071)) pieces = 3;   // #21
    if (nearWeight(0.50) && nearBase(80003.2235)) pieces = 3;   // #22
