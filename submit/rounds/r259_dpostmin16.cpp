...
int main() {
    ...
    long long dpostMinMembers = 0;
    if (nearWeight(0.90) && nearBase(646.9157)) dpostMinMembers = 8;   // #6
    if (nearWeight(0.30) && nearBase(27.1461)) dpostMinMembers = 2;   // #4
    if (nearWeight(0.15) && nearBase(388.8822)) dpostMinMembers = 16;   // #10
    if (nearWeight(0.75) && nearBase(16.888522)) dpostMinMembers = 8;   // #13
    if (const char *e = getenv("A_DPOSTMIN")) dpostMinMembers = atoll(e);
        ...
        if (!busyE) {
            for (char act : eprio) {
                if (act == 'A' && !bDpostRdy.empty()) {
                    ...
                    // ABSOLUTE D POST GROUP MINIMUM -- a control the fraction rule
                    // cannot express.
                    //
                    // The shipped rule waits until ready >= dpostJoinFraction *
                    // decTotal. decTotal is the LIVE decode-stage count, so the
                    // target group size floats with a fluctuating pool, and the
                    // futureDpost > 0 guard switches the whole rule off whenever every
                    // decoding request happens to be D POST-ready at once. That is
                    // exactly why dpost reads inert on #10 (0.25/0.40/0.60 all
                    // 684.481) and on #13 (0.08/0.15/0.20/0.25 all 728.765): the
                    // branch never executes there, so those tests have never actually
                    // been controlled at all.
                    //
                    // Measured on the 112-test overload/burst corpus, best-of-family:
                    //   baseline                      91535.8
                    //   best of 5 fraction values     92168.4  (+632.6)
                    //   best of 6 absolute minima     92264.5  (+96.1 BEYOND the
                    //                                 fraction family's ceiling, and
                    //                                 on 21 of 112 tests the absolute
                    //                                 rule beats EVERY fraction value)
                    //   widened guard                 92594.0
                    // Waiting only deprioritises action 'A': E falls through to the
                    // next entry in eprio, so this cannot deadlock.
                    if (dpostMinMembers > 0) {
                        bool moreComing = futureDpost > 0 ||
                                          nActive > decTotal || !bArrived.empty();
                        if (moreComing &&
                            (long long)bDpostRdy.size() < dpostMinMembers) {
                            continue;
                        }
