...
int main() {
    ...
    // #5 admits by age/service ratio rather than shortest-service-first. On
    // t5_fit, on top of r17's deferred join, this is tp 0.800300 -> 0.819240
    // (+2.4%) with tpot 69.277 -> 68.516. The same proxy understated the last
    // two #5 changes by roughly 1.8x (predicted +7.4%, judge paid +13.2%), and
    // #5 needs +4.7% to clear 500. Gated to w_tp == 0.80.
    char order = (w_c > 0.0 && !legacyQuarter) ? 'S' : 'F';
    if (targetTest5 && getenv("A_ORDER") == nullptr) order = 'A';
