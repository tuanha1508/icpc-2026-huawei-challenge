...
int main() {
    ...
    for (;;) {
        ...
        if (!busyE && useMarginal) {
            ...
            // TDR/TPOT BALANCE. costTdr and costTpot are normalised by DIFFERENT
            // denominators -- requestDenom = tdrCnt + pendCnt versus
            // gapDenom = gapCnt -- so their relative scale is arbitrary rather
            // than derived. Solving the SLOs from judge data shows TDR is the
            // dominant source of dist on essentially every latency-scored test:
            //   #3 #4 #5 #13 #14 TDR-only; #8 92% TDR; #7 65%; #6 53%.
            // TPOT dominates on #19 alone, which has w_c = 0 and so is unscored
            // on latency. The exponent of this damping was swept at r115; the
            // BALANCE between the two terms never has been. Weight TDR down 3x, to bracket the sign.
            costTdr *= max(0.0, 1.0 - normCNow) * 0.33;
