...
int main() {
    ...
    // ==================================================================
    // WAITING, SPLIT BY WHAT EACH TEST ACTUALLY SCORES (LESS waiting).
    //
    // Waiting for a bigger group buys E efficiency and costs two different
    // things: it raises tpot (a LATENCY cost) and it delays the last token
    // (a THROUGHPUT cost). Past sweeps moved one knob from the current point;
    // these ask what each test's own weights say about that trade.
    //   #7   w_tp 0.00 -> throughput is worth nothing, and latency is worth
    //                    248.90 points per unit dist (dist 0.336, dist_base
    //                    4.02) -- the highest latency leverage on the board.
    //                    Waiting only raises tpot here, so it should never wait.
    //   #19  w_c  0.00 -> latency is worth literally nothing, but w_tp is 1.00,
    //                    so the delay-the-last-token cost lands at full weight.
    //                    Bracketed both ways across this pair.
    //   #17  c_room 102 and only 7 points of tp room -> dist is the whole game.
    //   #5 #16 #22 #21  -> 0.006-0.17 points per unit dist: latency is nearly
    //                    free, so more batching is close to a one-way bet
    //                    PROVIDED the makespan does not grow.
    // Gates are independent: a losing cell costs only its own test.

    if (nearWeight(0.00) && nearBase(4.0177)) dgfrac = 0.0;   // #7
    if (nearWeight(0.67) && nearBase(3259.1504)) dgfrac = 0.0;   // #17
    if (nearWeight(1.00) && nearBase(41341.873)) dgfrac = 0.0;   // #19
    if (nearWeight(0.50) && nearBase(2917.9071)) dgfrac = 0.0;   // #21
    ...
    if (nearWeight(0.67) && nearBase(3259.1504)) dpostJoinFraction = 0.0;   // #17
    if (nearWeight(1.00) && nearBase(41341.873)) dpostJoinFraction = 0.0;   // #19
