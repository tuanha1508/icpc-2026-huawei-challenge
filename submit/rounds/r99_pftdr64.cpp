...
int main() {
    ...
    bool useMarginal = !(nearWeight(0.05) ||                          // #9 class
                         ...
                         legacyQuarter);
    double pfTdr = 64;
    if (const char *e = getenv("A_PFTDR")) pfTdr = atof(e);
    ...
    for (;;) {
        ...
        if (!busyE && useMarginal) {
            ...
            // Prefill is structurally under-weighted here. Decode is valued at
            // group * costTpot, where the group routinely holds tens of
            // requests, while prefill is valued at 1 * costTdr. The only
            // multiplier prefill gets is prefillBoost -- and that rides on
            // costTp, so on a w_tp == 0 test (costTp == 0) it multiplies
            // nothing and prefill keeps a bare unit weight. Those are exactly
            // the tests whose whole score is 1000 * norm_c, where admitting
            // requests sooner is the only lever on mean TDR. #3 sits at
            // norm_c 0.5006 with 499 points on it. pfTdr scales the latency
            // channel of prefill so the boost applies there too; 1.0 is the
            // incumbent behaviour exactly.
            if (!bPostRdy.empty()) {
                int rid = bPostRdy.v.front();
                consider(2, costTdr * pfTdr + costTp * averageOutput * prefillBoost,
                         col[2].at(lenIn[rid]));
            }
            if (!bArrived.empty() && nActive < Ntarget) {
                int rid = bArrived.v.front();
                consider(3, costTdr * pfTdr + costTp * averageOutput * prefillBoost,
                         col[0].at(lenIn[rid]));
            }
            if (bestAction >= 0) {
