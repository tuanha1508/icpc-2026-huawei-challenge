...
int main() {
    ...
    // everywhere else.
    // PER-TEST INTERACTION. #6's dgfrac peak of 0.25 was established while #6
    // ran dpost = 0.0. It now runs dpost 0.20 (+11.4 total), and dgfrac x dpost
    // is a known interaction, so that peak was measured under conditions that
    // no longer hold. Retest the other side of it.
    double dgfrac = immediateDecodeWaves ? 0.0 : (targetTest6 ? 0.40 : 0.24);
