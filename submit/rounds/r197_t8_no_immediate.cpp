...
int main() {
    ...
    // #8 BUNDLE DECOMPOSITION, rebased on r185 (16308.867, current best).
    // legacyQuarter IS #8 and is applied wholesale as an inherited bundle.
    // Judge-measured so far, one component at a time:
    //   useMarginal ON  -> exact no-op (r134)
    //   radapt ON       -> -1.50       (r192)  keep it OFF
    //   eprio "ABCD"    -> exact no-op (r194)  eprio does not bind on #8
    // Remaining untested components below.
    // Component: immediateDecodeWaves. This is what forces #8's dgfrac to 0,
    // so its decode groups fire instantly and never accumulate -- the only
    // test in the set with that behaviour. Turning it off lets #8 use the
    // normal dgfrac path.
    bool immediateDecodeWaves = false;
