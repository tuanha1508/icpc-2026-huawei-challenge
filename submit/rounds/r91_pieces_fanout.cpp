...
int main() {
    ...
    // r91 -- FIVE bits: prefill piece splitting, never probed on any of these.
    // `pieces` splits the P PROC layer range; more pieces means finer
    // interleaving but an extra S per piece. Judge record elsewhere: #6 lost
    // (r33), #4 lost via the r79 bundle. Untested on #9 #12 #13 #15 #17.
    if (getenv("A_PIECES") == nullptr) {
        if      (nearWeight(0.05) && nearBase(33.8523))   pieces = 2; // #9
        else if (nearWeight(0.99) && nearBase(4.4903))    pieces = 2; // #12
        else if (nearWeight(0.75) && nearBase(16.8885))   pieces = 2; // #13
        else if (nearWeight(0.45) && nearBase(180.3303))  pieces = 2; // #15
        else if (nearWeight(0.67) && nearBase(3259.1455)) pieces = 2; // #17
    }
