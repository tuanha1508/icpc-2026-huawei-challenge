...
int main() {
    ...
    // #10 DECOMPOSITION, rebased on r204 (16319.510, current best).
    // 315.5 points -- the largest headroom still open -- and the least explored
    // big test: only 7 distinct scores in 45 configs, spread 40.
    // w_tp 0.15, dist 97% TDR-driven, mean_tdr 140x SLO1.
    // Its bundle (cxT10) is only TWO settings and neither has ever been varied.
    // Settled already: marginal ON -0.011; dpost 0.40 did not bind (r185 mux).
    // Component: rprio 'D' -> 'P'. The GLOBAL default is 'P' and 'D' measured
    // -53.6 globally, yet #10 inherits 'D' while being 85% LATENCY-weighted
    // with dist 97% TDR -- and prefill is what closes TDR.
    // On #6 this same component was validated ('P' cost 2.44), so the axis is
    // live and test-specific.
    if (cxT10 && getenv("A_RPRIO") == nullptr) rprio = 'P';
