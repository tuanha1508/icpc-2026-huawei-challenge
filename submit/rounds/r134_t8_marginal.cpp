...
int main() {
    ...
    bool useMarginal = !((nearWeight(0.05) && !nearBase(33.8522)) ||  // #9 class
                         ...
                         // legacyQuarter IS #8 (nearWeight(0.25) && nearBase(10.8848)),
                         // 187.8 points and the largest block still on the legacy
                         // path. ONLY this useMarginal term is lifted -- #8 keeps
                         // every other legacyQuarter setting (dgfrac, eprio, dpost,
                         // balw), so this isolates the cost model alone.
                         (legacyQuarter && false));
