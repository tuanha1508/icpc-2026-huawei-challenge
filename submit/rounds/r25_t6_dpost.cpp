...
int main() {
    ...
    // #6 gets the same deferred join, and the reason it took this long to find
    // is worth recording. The response is NON-MONOTONIC: 0.0 baseline, a trough
    // at 0.25-0.5, then a sharp rise at 0.9. I sampled 0.5, saw all four
    // reproductions agree it lost, and wrote dpost off for #6 -- but 0.5 is the
    // bottom of the dip. All four now agree on 0.9:
    //   t6_fit3  367.713 -> 383.231   t6_fit2  355.235 -> 391.071
    //   t6_fit   396.853 -> 416.003   t6_flat  881.555 -> 999.063
    // The other reason it was missed: r03 measured -19.78 for a BUNDLE of
    // dpost 1.0 + marginal off + pieces 4, and marginal off disables the
    // prefillBoost = 14 that #6 depends on. dpost alone was never isolated.
    if (targetTest6 && getenv("A_DPOSTFRAC") == nullptr) dpostJoinFraction = 0.9;
