...
int main() {
    ...
    // #8 BUNDLE DECOMPOSITION. legacyQuarter is described in this file as
    // "an eight-site compatibility bundle inherited from the reference baseline" and is
    // applied to #8 wholesale. Of its seven live settings only ONE has been
    // tested individually (useMarginal, r134: no-op); balw's two arms are now
    // identical so that site is dead code. #8 has 187.8 points of headroom,
    // 112.9 of it latency with dist 92% TDR-driven. Disable one component.
    // Component: radapt is OFF for #8 purely because of the bundle.
    // The adaptive-concurrency loop steers N on the two measured excesses,
    // and #8's dist is 92% TDR -- exactly what it steers on.
    bool radapt = !targetTest5 && (getenv("A_RUSE") == nullptr);
