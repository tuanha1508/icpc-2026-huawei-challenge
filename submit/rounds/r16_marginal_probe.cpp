...
int main() {
    ...
    // MARGINAL EXCLUSION PROBE. This list is inherited and nothing has
    // re-validated it. It disables the marginal E-scheduler on #9 (0.05),
    // #10 (0.15), #8 (0.25), #4 (0.30), #15 (0.45), #5 (0.80), #16 (0.98) --
    // and #9/#10/#8/#4/#15 hold 1142 open points between them and have never
    // moved. #5 and #16 are left excluded because both are currently tuned and
    // scoring well. Each re-enable below is gated to one test, so the per-test
    // scores attribute them independently.
    bool useMarginal = !(nearWeight(0.80) || nearWeight(0.98));
    ...
    for (;;) {
        ...
        // PROBE: #8 is the only test on legacyQuarter and #15 the only one on
        // legacyDecodeFirst. Both overrides run AFTER the marginal block and
        // overwrite whatever it chose, so lifting them is the only way to test
        // those paths at all. 235 and 128 open points respectively.
        if (legacyQuarter && !eprioForced && getenv("A_LEGQ") != nullptr)
            eprio = "CDAB";
        if (legacyDecodeFirst && getenv("A_LEGD") != nullptr)
            eprio = "ABCD";
