...
int main() {
    ...
    // lost 650 overall.
    if (nearWeight(0.90) && nearBase(646.9157)) dpostJoinFraction = 0.45;   // #6
    // r298 PROBE.  r297 took #6's dpostJoinFraction from 0.20 to 0.00 and lost 11.40, so the
    // hold is EARNING its keep there -- and it is the only live batching lever
    // left on #6, whose D PRE side is an exact null (r294).  Walking up.
    // Not to 1.00: that value cost #5 42.19 on the same knob.
