...
int main() {
    ...
    // r92: pool size at or above which patience is raised to 0.90. 0 disables
    // (exact r78 behaviour).
    long long dpostFatPool = 0;
    if (const char *e = getenv("A_DPFAT")) dpostFatPool = atoll(e);
        ...
        if (!busyE) {
            for (char act : eprio) {
                if (act == 'A' && !bDpostRdy.empty()) {
                    ...
                    // r92: RAISE patience when the decode pool is fat. This is the
                    // generalisation of the only real win we have found: #22 gained
                    // **+36.214** from dpost 0.90 (tpot -24.9%), #16 +0.530
                    // (tpot -37.2%), while thin-pool tests lost (#17 +49.4% tpot,
                    // -27.310). The win condition is "the pool can FILL a bigger
                    // group", and `dpostPool` measures exactly that at runtime.
                    //
                    // This matters because the ranking is the mean of the 20 FROZEN
                    // tests, and all 20 of our nearBase() gates key on preliminary
                    // dist_base values that a frozen test will never match -- so the
                    // frozen set only ever sees the default path. A runtime rule is
                    // the ONLY way a per-test discovery can reach the real score.
                    //
                    // Note r86 tested the opposite direction (lower patience on thin
                    // pools) which was a no-op on the default path, since the default
                    // is already 0.05. Raising it has never been tried.
                    double djf = dpostJoinFraction;
                    if (dpostFatPool > 0 && dpostPool >= dpostFatPool && djf < 0.90)
                        djf = 0.90;
