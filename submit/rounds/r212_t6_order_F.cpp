...
int main() {
    ...
    // Component: order. #6 runs 'S' (SJF) like every w_c > 0 test. SJF minimises
    // MEAN flow time, but #6 is 90% THROUGHPUT-weighted, where the mean is not
    // what is scored -- tp depends only on the makespan. FIFO starves nobody.
    char order = ((nearWeight(0.90) && nearBase(646.9157)) ? 'F'
                 : ((w_c > 0.0) ? 'S' : 'F'));
