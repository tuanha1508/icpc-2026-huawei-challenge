...
int main() {
    ...
    for (;;) {
        ...
        auto admitOne = [&]() {
            ...
            int best = 0;
            if (balw < 0.0) {
                // balw < 0 balances remotes by CURRENT request count, and the
                // judge says that policy beats every tuning of the work-weighted
                // formula (balw -1 16303.718 > 2 16302.965 > 4 16301.792 >
                // 0 16276.887). But strict `<` sends every tie to the
                // lowest-indexed remote, biasing dispatch toward remote 0.
                // Keep count as the primary criterion -- it is the one that
                // wins -- and use accumulated work only to resolve ties.
                for (int k = 1; k < ruse; ++k)
                    if (load[k] < load[best] ||
                        (load[k] == load[best] && procWork[k] < procWork[best]))
                        best = k;
            } else {
