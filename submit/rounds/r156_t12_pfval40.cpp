...
int main() {
    ...
    for (;;) {
        ...
        if (!busyE && useMarginal) {
            ...
            // #12 is engine-bound and PREFILL-dominated (mean_tdr 1.25M with empty
            // remote queues). prefillBoost scales how much the marginal model
            // values a prefill action against decode on E -- i.e. it is the
            // direct control over how E splits its one-task-per-frame budget
            // between P PRE/P POST and the D PRE/D POST that emit tokens.
            // Never tested per-test. costTp on #12 is enormous
            // (1/(tp_UB - tp_base) = 62500), so this term dominates its cost
            // model and the balance is worth probing. More prefill: clear the backlog faster.
            double prefillBoost = (fabs(w_tp - 0.99) < 1e-9
                                   && fabs(dist_base / 4.490298 - 1.0) < 1e-3)
                                ? 40.0 : 14.0;
