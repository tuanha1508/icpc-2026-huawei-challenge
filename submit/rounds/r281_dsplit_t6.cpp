...
int main() {
    ...
    // r281 PROBE.  The conditional prefill-chunk rule (split P PROC so D PROC
    // can interleave between pieces instead of waiting out one long block) has
    // never reached the judge: `dsplit` was env-only, so it has always been 0.
    // #6 is the documented case for it -- E-bound at 94% with P PROC = 192.897
    // ms stalling every decoder pinned to that remote -- and it is worth 584
    // points at w_tp 0.90.  Gated to #6 alone; the frozen path keeps 0.
    long long dsplitGate = 0;
    if (nearWeight(0.90) && nearBase(646.9157)) dsplitGate = 4;   // #6
