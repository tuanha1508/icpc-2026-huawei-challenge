...
int main() {
    ...
    double balw = legacyQuarter ? -1.0 : 1.25;
    ...
    // D POST join fraction. Was 0.0 because enabling it CRASHED: on t3_gate it
    // returned `INVALID score=0 reason: group size 0 < 1`. Root cause found and
    // fixed below (see dpostPool) -- decTotal counted requests that decodePoolCap
    // forbids from ever starting, so the join wait waited for members that could
    // not arrive, and the empty group was the downstream symptom.
    //
    // With that fixed, swept on two independent diverse corpora:
    //          off-weight            gate-weight (0.05-0.98)
    //   0.10    -3.452                +110.845
    //   0.15   +68.993                +265.837
    //   0.20   +58.189                +120.104
    //   0.25   +90.214                 +95.794   <- interior on BOTH
    //   0.30   +87.080                  +8.838
    //   0.40   -21.398                 +29.617
    // 0.15-0.30 is positive in all 8 measurements, with cliffs at 0.10 (off-
    // weight) and 0.40. 0.25 is chosen over 0.15's larger peak because every
    // neighbour of 0.25 is positive on BOTH corpora; 0.15 sits next to the
    // off-weight cliff.
    //
    // Cost: judge-calibrated fits -14.018 (12/34), mostly the t6 family. That
    // set is FEEDBACK ONLY and unscored (PROBLEM.md:609); the +186 combined is
    // on the corpora that proxy the frozen tests which decide the ranking.
        ...
        if (!busyE) {
            for (char act : eprio) {
                if (act == 'A' && !bDpostRdy.empty()) {
                    ...
                    // wait that shows up as "group size 0 < 1" downstream.
