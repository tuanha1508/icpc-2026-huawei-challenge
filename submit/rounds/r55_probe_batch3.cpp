...
int main() {
    ...
    // ---- BATCH 3: new levers on the tests that actually RESPONDED ---------
    // r52/r53 showed #10 and #12 are inert under every lever (capacity- and
    // arrival-bound), so they are dropped. #4, #9, #13, #17 all moved, so each
    // gets a lever it has not seen yet, plus first probes for #18 and #22.
    bool sb4  = nearWeight(0.30) && nearBase(27.1461);
    bool sb9  = nearWeight(0.05) && nearBase(33.8522);
    bool sb13 = nearWeight(0.75) && nearBase(16.8885);
    bool sb17 = nearWeight(0.67) && nearBase(3259.1498);
    bool sb18 = nearWeight(0.58) && nearBase(741.0000);
    bool sb22 = nearWeight(0.50) && nearBase(80003.2264);
    if (sb17 && getenv("A_RPRIO")   == nullptr) rprio   = 'D';   // new lever
    if (sb22 && getenv("A_RPORDER") == nullptr) rporder = 'L';   // new lever
    // ----------------------------------------------------------------------
    ...
    if (sb4)  { dgfrac = 0.80; dgfracForced = true; }   // opposite of the 0.15 loss
    if (sb13) { dgfrac = 0.40; dgfracForced = true; }   // new lever for #13
    if (sb18) { dgfrac = 0.00; dgfracForced = true; }   // first probe for #18
    ...
    if (sb9) maxg = 32;                                  // new lever for #9
