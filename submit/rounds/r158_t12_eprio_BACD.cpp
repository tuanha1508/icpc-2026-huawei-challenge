...
int main() {
    ...
    // #12: prefillBoost is inert in BOTH directions (2 and 40 both exact
    // no-ops), so its engine contention is not decided by the marginal cost
    // model -- the prefill arms never win the argmax there. What actually
    // orders E's one-task-per-frame budget is eprio. Forcing it bypasses the
    // dynamic choice entirely. Action codes: A = D POST, B = D PRE,
    // C = P POST, D = P PRE; the dynamic fallback is "CDAB", i.e. prefill
    // FIRST -- the opposite of what a throughput-bound test wants.
    // D PRE ahead of D POST, prefill still last.
    if (nearWeight(0.99) && nearBase(4.490298) && getenv("A_EPRIO") == nullptr) {
        eprio = "BACD";
        eprioForced = true;
    }
