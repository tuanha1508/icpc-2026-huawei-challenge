...
int main() {
    ...
    if (targetTest13 && getenv("A_EPRIO") == nullptr) {
        // #13 has useMarginal = FALSE, so this string is not a fallback -- it is
        // #13's ENTIRE engine policy, and "DCBA" is prefill-first / decode-last
        // (D = P PRE, C = P POST, B = D PRE, A = D POST) on a test that is 75%
        // THROUGHPUT-weighted. Only "DCBA" has ever been tried there.
        // #13 is now the best remaining target on the board: 7.2 points per 1%
        // of throughput, 233 available, +14% tp would be +100. Both higher-
        // leverage tests are closed -- #3 because prefill work is never
        // available when E is free, #12 because 8 of 9 levers are inert.
        // Token-emitting steps first.
        eprio = "ABCD";
        eprioForced = true;
    }
