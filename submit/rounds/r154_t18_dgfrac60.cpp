...
int main() {
    ...
    // #18 was the other test that gained from dgfrac 0.32 (+0.03). #12's curve
    // shows the lever keeps paying well past 0.32, so test whether #18 -- the
    // only other test with a positive dgfrac response -- behaves the same way.
    // #12 stays at its measured best (0.60) so any change is attributable to #18.
    if (nearWeight(0.58) && nearBase(740.988751)) dgfrac = 0.60;   // #18
