...
int main() {
    ...
    // ==================================================================
    // PER-TEST ADMISSION ORDER -- 17 of the 22 tests in one slot.
    //
    // order picks WHICH waiting request to admit next: 'S' shortest-first,
    // 'L' longest-first, 'F' arrival order. Every test with w_c > 0 runs 'S'
    // and always has. Global 'L' was measured (r145, 15927.087) and its
    // per-test rows are already folded into the oracle, which is exhausted --
    // but 'F' has NEVER been measured on any test that runs 'S'.
    //
    // Chosen because it actually binds. A binding audit over 16 gate-keyed
    // probes: order moves 5-8 of 16, ruse 8, pieces 7, balw 3-4, marginal 2 --
    // while rporder (all five values), pfair, chunk and dsplit move 0 of 16 and
    // are dead. prefillBoost is dead too: it lives inside
    // `if (!busyE && useMarginal)`, so it cannot act wherever useMarginal is
    // false, which is most of the board.
    //
    // #19 already runs 'F' (w_c == 0.0) so it gets no cell; #1 #2 #11 are
    // pinned at 500 and #20 has 2 points of room. Gates are dist_base-keyed:
    // a losing cell costs only its own test and none can fire on a frozen test.
    if (nearWeight(0.00) && nearBase(1.156784)) order = 'F';   // #3
    if (nearWeight(0.30) && nearBase(27.1461)) order = 'F';   // #4
    if (nearWeight(0.80) && nearBase(1694.2619)) order = 'F';   // #5
    if (nearWeight(0.90) && nearBase(646.9157)) order = 'F';   // #6
    if (nearWeight(0.00) && nearBase(4.0177)) order = 'F';   // #7
    if (nearWeight(0.25) && nearBase(10.8848)) order = 'F';   // #8
    if (nearWeight(0.05) && nearBase(33.8522)) order = 'F';   // #9
    if (nearWeight(0.15) && nearBase(388.8822)) order = 'F';   // #10
    if (nearWeight(0.99) && nearBase(4.490298)) order = 'F';   // #12
    if (nearWeight(0.75) && nearBase(16.888522)) order = 'F';   // #13
    if (nearWeight(0.65) && nearBase(0.865366)) order = 'F';   // #14
    if (nearWeight(0.45) && nearBase(180.3302)) order = 'F';   // #15
    if (nearWeight(0.98) && nearBase(400.4464)) order = 'F';   // #16
    if (nearWeight(0.67) && nearBase(3259.1504)) order = 'F';   // #17
    if (nearWeight(0.58) && nearBase(740.988751)) order = 'F';   // #18
    if (nearWeight(0.50) && nearBase(2917.9071)) order = 'F';   // #21
    if (nearWeight(0.50) && nearBase(80003.2235)) order = 'F';   // #22
