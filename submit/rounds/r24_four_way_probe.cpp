...
int main() {
    ...
    // PROBE #13: targetTest13 has pinned rprio = 'P' (prefer P PROC on a free
    // remote) since it was written, and 'D' has never been tried against it.
    // 271 open points. Everything else on #13 has been tuned.
    // (was: if (targetTest13 && getenv("A_RPRIO") == nullptr) rprio = 'P';)
    ...
    bool dgfracForced = targetTest13;
    {
        ...
        if (nearWeight(0.98) && w_tp > w_c && ratio > 0.0 && ratio < 1.15)
            dgfrac = 0.95;
    }
    ...
    if (getenv("A_DGFRAC") == nullptr) {
        if (nearWeight(0.30)) dgfrac = 0.60;              // transient by design
        else if (nearWeight(0.98)) dgfracForced = true;   // persist the 0.95
        // PROBE #9 / #10 / #12. A_DGFRAC always set dgfracForced too, so on the
        // useMarginal=false tests only FORCED widths were ever tried, never
        // seeds -- and on #4 those differ by 7.4 points in OPPOSITE directions.
        // #12 is different again: useMarginal is true there so its dgfrac has
        // been pinned at 0.25 since forever and never varied at all.
        else if (nearWeight(0.05)) dgfrac = 0.45;         // #9,  264 open
        else if (nearWeight(0.15)) dgfrac = 0.45;         // #10, 316 open
        // #12 dgfrac 0.60 was built and dropped before shipping: the calibrated
        // t12_fit said +0.24 but t12_het said -10.14. Proxies disagreeing is the
        // signal that cost me the layered-prefill round, so it does not ship.
    }
    ...
    // PROBE #8: legacyQuarter pins balw = -1, i.e. balance remotes by raw request
    // COUNT rather than estimated queued work. #8 is the only test on that path,
    // it holds 235 open points, and every other legacy component there --
    // dgfrac, eprio, marginal, legacyDecodeRemote -- returned exactly 0.00.
    // This is the one that was never tried.
    double balw = 4.0;
