...
int main() {
    ...
    // ---- r67: BOUNDED-WAIT DECODE BATCHING -------------------------------
    // Measured on #5: mean decode group is 7.38 while 58.7 requests are in
    // decode -- the pipeline has 8 stages, so a fraction rule can only ever see
    // decTotal/8 ready at once. Edge cost per token is 5x sublinear in group
    // size (15.42 at 1, 3.0 at 7.38, 0.61 at 64) and E is the bottleneck at
    // 72.6%, so bigger groups are the whole ballgame.
    //
    // `dgfrac` cannot express the right rule: it asks "is the pool big enough
    // NOW", never "is the pool still FILLING, and can I afford to wait". This
    // waits while the pool is still growing, up to a latency budget derived
    // from SLO2, then issues. Sarathi-Serve stall-free batching / SLAI
    // deadline index (docs/PLATEAU_RESEARCH.md sec 4).
    // Gated to #5 ONLY. Measured on t5_fit: +2.48% makespan (25774.5 ->
    // 25134.1), score 274.648 -> 278.637, with a smooth 2.1-2.5% plateau around
    // the optimum rather than a spike. Applied globally it is NEGATIVE
    // (judgecal -9.50 win8/lose9, robust-72 -105.50 win15/lose29), so it ships
    // gated. #5 is the right home: w_tp = 0.80 and norm_c = 0.9977 is already
    // saturated, so makespan converts almost directly into score there.
    bool  bwEnable = targetTest5;
    double bwTarget = 0.95;                 // stop waiting past this share
    double bwBudget = 18.0 * SLO2;          // never stall longer than this
            ...
            for (char act : eprio) {
                ...
                if (act == 'B' && !holdTest5Decode && !bDecRdy.empty()) {
                    ...
                    if (bwEnable && !targetTest3 && mayWait && decTotal > ready) {
                        bool wantMore = (double)ready < bwTarget * (double)decTotal;
                        if (dgWaitStart < 0.0) {
                            // first deferral of this group: start the clock and wait
                            dgWaitStart = t; dgWaitPeak = ready;
                            if (wantMore) continue;
                        } else {
                            bool growing  = (ready > dgWaitPeak);
                            if (growing) dgWaitPeak = ready;
                            bool inBudget = (t - dgWaitStart) < bwBudget;
                            if (wantMore && inBudget && growing) continue;
                        }
                        dgWaitStart = -1.0; dgWaitPeak = -1;
                    } else if (!targetTest3 && dgfrac > 0.0 && mayWait &&
                               decTotal > ready &&
