...
int main() {
    ...
    for (;;) {
        ...
        // Component: #10's eprio "CDBA" -> "CDAB". Both are prefill-first;
        // they differ only in decode order (D PRE vs D POST first). On #13 that
        // same swap was worth 23.89 points, so the axis is live in general.
        if (cxT10 && !eprioForced) eprio = "CDAB";   // #10
