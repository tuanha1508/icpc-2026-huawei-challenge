...
int main() {
    ...
    // everywhere else.
    // #6 refinement. It holds the largest headroom on the board (597 points,
    // w_tp 0.90) and its dgfrac curve has three judge points but has never been
    // refined between them: 0.05 -> -13.69, 0.25 -> current, 0.60 -> -77.53.
    // The peak is bracketed in [0.05, 0.60] and the fall-off toward 0.60 is far
    // steeper than toward 0.05, which puts the optimum below the midpoint.
    // dgfrac is KNOWN to bind on #6 from those judge measurements, so this is
    // not a speculative gate. Below current, toward the shallow side.
    double dgfrac = immediateDecodeWaves ? 0.0 : (targetTest6 ? 0.18 : 0.24);
