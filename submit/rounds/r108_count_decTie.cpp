...
int main() {
    ...
    for (;;) {
        ...
        auto admitOne = [&]() {
            ...
            int best = 0;
            if (balw < 0.0) {
                // As above, but resolve ties by the number of decoders pinned
                // to the remote. A remote is serial, so a long prefill blocks
                // every decoder on it -- decCnt targets that stall directly,
                // and it is the term balw 0 removed and lost 26.8 points for.
                for (int k = 1; k < ruse; ++k)
                    if (load[k] < load[best] ||
                        (load[k] == load[best] && decCnt[k] < decCnt[best]))
                        best = k;
            } else {
