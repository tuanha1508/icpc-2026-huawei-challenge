...
int main() {
    ...
    // MULTIPLEX: dpostJoinFraction = 0.60 on 15 tests. dpost is the ONLY productive knob (#6 +11, #7 +1.09, #22 +36.2 historically). Tests peak at very different values, so the curve is under-sampled: 0.05, 0.25 and 0.40 are done. 0.60 fills the high side. #6 #7 #22 excluded -- already at their own peaks.
    // Independent gates -- one slot, many experiments. Losers cost only
    // their own test and are dropped after the per-test harvest.
    if (nearWeight(0.00) && nearBase(1.156784)) dpostJoinFraction = 0.60;   // #3
    if (nearWeight(0.30) && nearBase(27.1461)) dpostJoinFraction = 0.60;   // #4
    if (nearWeight(0.80) && nearBase(1694.2619)) dpostJoinFraction = 0.60;   // #5
    if (nearWeight(0.25) && nearBase(10.8848)) dpostJoinFraction = 0.60;   // #8
    if (nearWeight(0.05) && nearBase(33.8522)) dpostJoinFraction = 0.60;   // #9
    if (nearWeight(0.15) && nearBase(388.8822)) dpostJoinFraction = 0.60;   // #10
    if (nearWeight(0.99) && nearBase(4.490298)) dpostJoinFraction = 0.60;   // #12
    if (nearWeight(0.75) && nearBase(16.888522)) dpostJoinFraction = 0.60;   // #13
    if (nearWeight(0.65) && nearBase(0.865366)) dpostJoinFraction = 0.60;   // #14
    if (nearWeight(0.45) && nearBase(180.3302)) dpostJoinFraction = 0.60;   // #15
    if (nearWeight(0.98) && nearBase(400.4464)) dpostJoinFraction = 0.60;   // #16
    if (nearWeight(0.67) && nearBase(3259.1504)) dpostJoinFraction = 0.60;   // #17
    if (nearWeight(0.58) && nearBase(740.988751)) dpostJoinFraction = 0.60;   // #18
    if (nearWeight(1.00) && nearBase(41341.873)) dpostJoinFraction = 0.60;   // #19
    if (nearWeight(0.50) && nearBase(2917.9071)) dpostJoinFraction = 0.60;   // #21
