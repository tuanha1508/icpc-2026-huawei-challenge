...
int main() {
    ...
    // FORCED WAVE WIDTH. #4 responded to a dgfrac seed of 0.6 with +2.76 on the
    // judge (tp 0.058849 -> 0.059922, +1.8%). But #4 has useMarginal false, so
    // fixedDecodeWaves is false and the adaptive rule dgfrac = 0.05 + 0.70*frac
    // overwrites the seed every 16-64 events -- that gain came from a transient.
    // Forcing it makes it permanent. The same adaptive rule governs #15 and #16,
