...
int main() {
    ...
    // ==================================================================
    // WEIGHT-ASYMMETRY EXPLOITATION (moderate).
    //
    // Derived from each test's own scoring weights, not from guessing. Cost of
    // one unit of dist, and what each test can therefore give away for free:
    //     #7   w_tp 0.00  -> THROUGHPUT IS WORTH NOTHING, and latency is worth
    //                       248.90 points per unit of dist (dist 0.336 on a
    //                       dist_base of only 4.02). Highest latency leverage
    //                       on the board: -0.04 dist is +10 points. It should
    //                       never wait for a group; it runs dgfrac 0.10 / dpost 0.25.
    //     #19  w_c  0.00  -> LATENCY IS WORTH LITERALLY NOTHING (0.0000 pts per
    //                       unit dist) with 80 points of pure throughput room.
    //                       It should batch as hard as possible; it runs
    //                       dgfrac 0.40 / dpost 0.25.
    //     #5  0.118 pts/unit, #21 0.171, #17 0.101, #16 0.050, #22 0.0062
    //                    -> latency is nearly free, so more batching is close
    //                       to a one-way bet.
    // Every one of these is a direction the per-test sweeps never took, because
    // they moved one knob from the current point instead of asking what the
    // test's weights allow. Gates are independent: a losing cell costs only
    // its own test.

    if (nearWeight(0.80) && nearBase(1694.2619)) dgfrac = 0.10;   // #5
    if (nearWeight(0.00) && nearBase(4.0177)) dgfrac = 0.05;   // #7
    if (nearWeight(0.98) && nearBase(400.4464)) dgfrac = 0.15;   // #16
    if (nearWeight(1.00) && nearBase(41341.873)) dgfrac = 0.60;   // #19
    if (nearWeight(0.50) && nearBase(2917.9071)) dgfrac = 0.60;   // #21
    if (nearWeight(0.50) && nearBase(80003.2235)) dgfrac = 0.15;   // #22
    ...
    if (nearWeight(0.00) && nearBase(4.0177)) dpostJoinFraction = 0.10;   // #7
    if (nearWeight(1.00) && nearBase(41341.873)) dpostJoinFraction = 0.60;   // #19
