...
int main() {
    ...
    // MULTIPLEXED: order 'F' (FIFO) on eight tests. Every test with w_c > 0
    // runs 'S' (SJF), which minimises MEAN flow time -- correct on average, but
    // dist is sqrt(ex_tdr^2 + ex_tpot^2) with each excess clamped at zero, so
    // it is nonlinear in the TAIL. FIFO starves nobody. 'L' and 'R' were both
    // measured globally and lost; plain FIFO has never been tried per-test.
    if ((nearWeight(0.00) && nearBase(1.156784)) ||   // #3
        (nearWeight(0.25) && nearBase(10.8848))  ||   // #8
        (nearWeight(0.05) && nearBase(33.8522))  ||   // #9
        (nearWeight(0.15) && nearBase(388.8822)) ||   // #10
        (nearWeight(0.45) && nearBase(180.3302)) ||   // #15
        (nearWeight(0.58) && nearBase(740.988751)) || // #18
        (nearWeight(0.50) && nearBase(2917.9071)) ||  // #21
        (nearWeight(0.50) && nearBase(80003.2235)))   // #22
        order = 'F';
