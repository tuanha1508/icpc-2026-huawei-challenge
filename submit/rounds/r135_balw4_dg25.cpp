...
int main() {
    ...
    // everywhere else.
    double dgfrac = immediateDecodeWaves ? 0.0 : (targetTest6 ? 0.25 : 0.25);
    ...
    // INTERACTION probe. Every config measured so far changed exactly one knob,
    // so the per-test oracle has never seen an interaction term -- a pair can
    // win on a test where neither single does. Binding verified globally
    // (16/25 and 14/25), which single-test gates cannot be.
