...
int main() {
    ...
    if (getenv("A_DPOSTFRAC") == nullptr) {
        ...
        // r79 PROBE -- the two remaining untested tests that pass the density
        // condition the judge established. dpost 0.90 pays only where the decode
        // pool can fill a bigger group; tdr/tpot is the proxy for that ratio:
        ...
        else if (nearWeight(0.30) && nearBase(27.1460)) dpostJoinFraction = 0.90; // #4
        else if (nearWeight(0.75) && nearBase(16.8885)) dpostJoinFraction = 0.90; // #13
    }
