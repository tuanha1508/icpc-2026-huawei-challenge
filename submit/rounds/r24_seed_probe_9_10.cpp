...
int main() {
    ...
    if (getenv("A_DGFRAC") == nullptr) {
        ...
        // PROBE #9 and #10. Every A_DGFRAC sweep I ran also set dgfracForced,
        // so on the useMarginal=false tests only FORCED widths were ever tried,
        // never SEEDS -- and #4 shows the two differ by 7.4 points IN OPPOSITE
        // DIRECTIONS. #9 and #10 have never had a seeded width, so their
        // recorded nulls may be an artifact of that. Both are latency-weighted
        // (w_c 0.95 and 0.85), so the seed is kept modest.
