...
int main() {
    ...
    double nfactor = 1.0;
    // MULTIPLEXED: nfactor 2.0 on the latency-weighted tests. The admission cap
    // only exists where w_c >= w_tp, so these are the only tests it can reach.
    // Judge-measured globally: nf 0.5 and 0.75 -> 16134, nf 4 -> -13.7 (all on
    // #15), nf 1 optimal. But nfactor has never been set PER TEST, and the
    // global optimum need not be every test's optimum.
    // nearWeight/nearBase are declared later (247/265), so inline them here.
    {
