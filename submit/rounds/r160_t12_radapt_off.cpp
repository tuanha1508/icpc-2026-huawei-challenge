...
int main() {
    ...
    // #12: radapt is the adaptive-concurrency feedback loop that steers N on
    // the two measured excesses. #12 is 99% throughput-weighted, so steering on
    // dist (worth 3.5 points in total there) can only distort a schedule whose
    // score is essentially pure tp. Turning it off for #12 alone leaves the
    // static Little's-Law seed in place without the latency feedback. radapt
    // has never been probed per-test.
