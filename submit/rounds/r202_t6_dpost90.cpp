...
int main() {
    ...
    // #6 dpost curve. It came from dpost = 0.0 -- the ONLY test set to zero --
    // and 0.40 measured **+2.290**, raising tp from 0.728213 to 0.733537
    // (+0.73%). At 3.1 points per 1% of throughput that predicts +2.27, which
    // matches: a real THROUGHPUT gain, not a latency trade.
    // The curve is wide open from 0, and the single largest per-test win in the
    // project is #22's dpost 0.90 (+36.2). Test the same extreme that #22 rewarded.
    if (nearWeight(0.90) && nearBase(646.9157)) dpostJoinFraction = 0.90;   // #6
