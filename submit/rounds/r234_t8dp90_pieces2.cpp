...
int main() {
    ...
    int pieces = 1;
    // MULTIPLEX: pieces = 2 on all 17 other gateable tests, COMBINED with #8's
    // dpost refinement so one slot carries 18 experiments rather than one.
    // Splitting prefill lost everywhere GLOBALLY (-346 at pieces 2, -846
    // adaptive) but has NEVER been gated per-test, and the mechanism -- letting
    // D PROC interleave between pieces -- is real.
    // `pieces` is declared before nearWeight/nearBase (247/265), so the
    // predicates are inlined here exactly as they are defined there.
    {
