...
int main() {
    ...
    // MULTIPLEX: rporder = 'S' on 8 tests. rporder 'S' = prefill-queue shortest-first. Only #7 and #13 carry it; it binds only 2/60 globally because most tests never queue >1 prefill, so per-test is the only way to find where it matters.
