...
int main() {
    ...
    for (;;) {
        ...
        if (!busyE && useMarginal) {
            ...
            // forcePrefill is the solver's ONLY mechanism for prioritising TDR,
            // and it fires only when noGaps -- gapCnt == 0, i.e. workloads where
            // every request emits a single token. That is the narrowest possible
            // trigger. Solving the SLOs from judge data shows TDR is the
            // dominant source of dist on essentially every latency-scored test
            // (#3 #4 #5 #13 #14 TDR-only, #8 92%, #7 65%, #6 53%), with or
            // without gaps. The natural trigger is "TDR is the binding excess",
            // which is already computed two lines above as exTdr / exTpot.
            // Note scaling costTdr directly does NOT work: it appears only in
            // the prefill arms of the argmax, which win ~1.1% of decisions --
            // the same reason pfTdr was inert. This changes the ORDER instead.
            // Wider: drop the w_c >= w_tp guard so throughput-weighted tests whose dist is still TDR-driven (#5, #6) are covered too.
            bool forcePrefill = ((noGaps || exTdr > exTpot) && !legacyHalfNoGaps) &&
