...
int main() {
    ...
    // RE-TEST under the new remote arbitration. r16 measured these exclusions
    // on the judge with rprio = 'D' and got #4 -20.73, #15 -4.76, #9 -3.94,
    // #8 0.00, #10 +0.06, so the list was kept. r27 then flipped rprio to 'P',
    // which changes what the marginal scheduler is choosing between, and the
    // sign appears to have flipped with it -- re-measured on 12 diverse
    // workloads per weight: w 0.30 now +57.18, w 0.25 now +10.80.
    // Only those two are re-enabled; each is gated to a single judge test so
    // the per-test scores attribute them separately and the downside is capped
    // at that one test.
