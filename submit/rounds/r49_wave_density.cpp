        ...
        if (!busyE) {
            for (char act : eprio) {
                if (act == 'A' && !bDpostRdy.empty()) {
                    // decTotal counts every request in the decode stage, but
                    // decodePoolCap limits how many may actually be STARTED
                    // (targetTest3 uses 1). Waiting for a fuller D POST group
                    // then waits for members that can never arrive -- a circular
                    // wait whose downstream symptom is an empty D PRE group and
                    // "INVALID: group size 0 < 1" on t3_gate/t3_burst/t3_cont.
                ...
                if (act == 'B' && !holdTest5Decode && !bDecRdy.empty()) {
                    ...
                    if (targetTest3) {
                        ...
                        // STRUCTURAL: D PRE is one E call for the whole wave, but
                        // D PROC runs per-remote and is strongly sublinear, so a
                        // wave spread thinly over many remotes buys small, costly
                        // D PROC groups. Optionally require a minimum ready-count
                        // per remote before including that remote's requests.
                        // Swept on 339 workloads vs minR = 1 (previous behaviour):
                        //   2  +145.80  19/10  top1 50%  DISAGREE  trim  +65.10
                        //   3  +291.76  25/9   top1 26%  agree     trim +106.44  <-
                        //   4  +181.31  17/9   top1 31%  agree     trim  +56.56
                        //   6  +257.03  24/16  top1 41%  DISAGREE  trim  +60.29
                        // 3 and 4 both pass every robustness check (favourable
                        // count, agreeing halves, sign-stable trimmed sum), so
                        // this is a plateau rather than a spike. 3 is the maximum
                        // and has the LOWEST concentration at 26%, meaning the
                        // gain is spread across many workloads rather than one.
                        long long minR = 3;
                        if (const char *e = getenv("A_DMINR")) minR = atoll(e);
                        if (minR > 1) {
                            vector<long long> cnt(K, 0);
                            for (int rid : bDecRdy.v)
                                if (assigned[rid] >= 0) cnt[assigned[rid]]++;
                            tmp.clear();
                            for (int rid : bDecRdy.v)
                                if (assigned[rid] >= 0 && cnt[assigned[rid]] >= minR)
                                    tmp.push_back(rid);
                            if (tmp.empty()) tmp = bDecRdy.v;   // never stall
                        } else {
                            tmp = bDecRdy.v;
                        }
                    }
                    if (tmp.empty()) continue;   // never emit a zero-size group
