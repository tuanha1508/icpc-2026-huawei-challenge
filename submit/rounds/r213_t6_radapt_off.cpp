...
int main() {
    ...
    // #6, rebased on r204 (16319.510, current best). Component scorecard, all
    // judge-measured one at a time:
    //   dpost   peak 0.25  **+11**      dgfrac  peak 0.25 (both sides)
    //   pfval   14 (28 -5.55)           rprio   'D' ('P' -2.44)
    //   balw    -1 (4 -0.10)            marginal ON (OFF -19.7)
    //   eprio   forced CDBA = no-op
    // Only dpost has EVER moved #6 upward.
    // Component: radapt. The adaptive-concurrency loop steers N on the two
    // measured excesses, i.e. on dist -- which is worth 10% of #6's score
    // against throughput's 90%. Steering on the minor term may distort a
    // schedule that should be maximising tp. Never tested for #6.
    bool radapt = !targetTest5 && !legacyQuarter
                  && !(nearWeight(0.90) && nearBase(646.9157))
                  && (getenv("A_RUSE") == nullptr);
