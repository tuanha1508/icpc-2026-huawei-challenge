...
int main() {
    ...
    // Decode group size is UNBOUNDED by default (4e18). dgfrac 0.32 measured
    // -13.60 on the judge against 0.18, i.e. over-large decode groups are the
    // expensive direction, and nothing currently caps the extreme tail.
    long long maxg = probeT12 ? 1 : (targetTest12 ? 8 : (long long)8);
