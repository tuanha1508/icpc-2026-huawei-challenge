...
int main() {
    ...
    // EVERY-TEST PROBE #3. dgfrac and maxg are now dead per-test (r247 moved
    // 1 of 22), and dpost is tuned wherever it binds. rprio has never been
    // varied per-test on anything except #6 #10 #13 -- every other test simply
    // inherits the global 'P'. Flip it to 'D' (decode-first on a free remote).
    if (nearWeight(0.00) && nearBase(1.156784)) rprio = 'D';   // #3
    if (nearWeight(0.30) && nearBase(27.1461)) rprio = 'D';   // #4
    if (nearWeight(0.80) && nearBase(1694.2619)) rprio = 'D';   // #5
    if (nearWeight(0.05) && nearBase(33.8522)) rprio = 'D';   // #9
    if (nearWeight(0.15) && nearBase(388.8822)) rprio = 'D';   // #10
    if (nearWeight(0.99) && nearBase(4.490298)) rprio = 'D';   // #12
    if (nearWeight(0.65) && nearBase(0.865366)) rprio = 'D';   // #14
    if (nearWeight(0.45) && nearBase(180.3302)) rprio = 'D';   // #15
    if (nearWeight(0.98) && nearBase(400.4464)) rprio = 'D';   // #16
    if (nearWeight(0.58) && nearBase(740.988751)) rprio = 'D';   // #18
    if (nearWeight(1.00) && nearBase(41341.873)) rprio = 'D';   // #19
    if (nearWeight(0.50) && nearBase(2917.9071)) rprio = 'D';   // #21
    if (nearWeight(0.50) && nearBase(80003.2235)) rprio = 'D';   // #22
