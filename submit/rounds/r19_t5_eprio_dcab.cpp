...
int main() {
    ...
    // #5: put D (admit a new request) first in the E order. On t5_fit, on top
    // of r17's deferred join, DCAB / DACB / ADCB all give tp 0.800300 ->
    // 0.813326 (+1.6%) while every C-first order stays flat -- admitting sooner
    // widens the decode pool that later waves draw from. eprio is a PROVEN-live
    // mechanism on #5: removing the inherited "CDBA" was worth +13.89 on the
    // judge and the deferred join a further +34.15. Gated to w_tp == 0.80.
