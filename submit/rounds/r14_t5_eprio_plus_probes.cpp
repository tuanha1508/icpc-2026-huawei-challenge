...
int main() {
    ...
    // which is precisely why their tp is 1.065646 against our 1.008309 while
    // tdr (1497.254452) and dist (3.835809) match to six decimals. Default
    // "CDAB" restores D POST first.
    ...
    //   #8  (w_tp 0.25): legacyQuarter forces dgfrac 0, firing a decode wave at
    //       the first ready request. Give it 0.25 so E's 2S + dpre + dpost is
    //       amortised over a real group. 235 pts sit behind that legacy path.
    double dgfrac = immediateDecodeWaves ? 0.0 : 0.25;
    if (legacyQuarter && getenv("A_DGFRAC") == nullptr) dgfrac = 0.25;
    {
    ...
    if (getenv("A_DPOSTFRAC") == nullptr &&
        (nearWeight(0.05) || nearWeight(0.30) || nearWeight(0.67)))
        dpostJoinFraction = 0.75;
