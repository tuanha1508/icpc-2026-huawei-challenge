...
int main() {
    ...
    // #12 has NO admission cap: the cap is gated on w_c >= w_tp, false at
    // w_tp 0.99. So every arrival is admitted immediately and the engine
    // spends its serial capacity on P PRE for new requests instead of the
    // D PRE / D POST that actually emit tokens. Capping globally was ruinous
    // (-1316), but #12 is precisely the flooded, prefill-bound case where
    // throttling admission should raise throughput.
    // nearWeight/nearBase are declared below this point, so inline them:
    if ((w_c >= w_tp || (fabs(w_tp - 0.99) < 1e-9
                         && fabs(dist_base / 4.490301 - 1.0) < 1e-3))
        && nfactor > 0.0) {
