...
int main() {
    ...
    // #13 balances better on raw remote counts than on estimated queued work:
    // measured on a fitted #13 reproduction (err 0.0071 against the judge's
    // tp/tpot/tdr/dist) dist falls 2.5672 -> 2.4213 with tp unchanged, worth
    // 0.1459/16.888 * w_c 0.25 = +2.16 points. Gated so nothing else moves.
    double balw = (legacyQuarter || targetTest13) ? -1.0 : 4.0;
