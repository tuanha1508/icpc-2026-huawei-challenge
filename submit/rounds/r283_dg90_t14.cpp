...
int main() {
    ...
    // r283 PROBE.  #14 carries the steepest score gradient in the set -- its
    // tp window is only 0.42% wide -- but nothing has ever moved it, so the
    // first question is whether its schedule responds to decode dispatch AT
    // ALL.  This is a deliberately large perturbation: dgfrac 0.40 -> 0.90.
    // useMarginal is true for #14, so fixedDecodeWaves is true and the static
    // gate is what actually runs (the dynamic 0.05 + 0.70*frac rule is dead
    // here).  A near-zero delta says #14 is arrival-span bound and I stop
    // spending slots on it; a large one says the makespan is in play.
    if (nearWeight(0.65) && nearBase(0.865366)) { dgfrac = 0.90; dgfracForced = true; }   // #14
