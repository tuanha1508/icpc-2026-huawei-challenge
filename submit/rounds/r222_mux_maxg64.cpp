...
int main() {
    ...
    if (const char *e = getenv("A_MAXG")) {
        long long v = atoll(e);
        if (v > 0) maxg = v;
    // MULTIPLEX: maxg = 64 on 10 tests. maxg is UNBOUNDED (4e18) on every test except #12. A finite cap has never been gated to any of them; maxg 8 was globally -1031, but 64 is mild and per-test caps are untested.
    // Independent gates -- one slot, many experiments. Losers cost only
    // their own test and are dropped after the per-test harvest.
