...
int main() {
    ...
    // #6, rebased on r204 (16319.510, current best).
    // Settled so far: dpost peak 0.25 (+11) | dgfrac peak 0.25 | pfval 14 (28
    // costs 5.55) | rprio 'D' correct ('P' -2.44) | balw -1 correct (4 -0.10)
    // Component: useMarginal. #6 is NOT in the exclusion list, so it runs the
    // marginal cost model -- and that model has never been turned off for it.
    // #6 is 90% throughput-weighted, and the model's own instrumentation shows
    // costTp is undamped there while prefill arms win ~1% of decisions, so the
    // greedy rate-maximiser may simply be the wrong policy for this test.
    bool useMarginal = !((nearWeight(0.90) && nearBase(646.9157)) ||   // #6 probe
