...
int main() {
    ...
    // #10 DECOMPOSITION. 315.5 points of headroom -- the largest still open --
    // and the LEAST explored big test: only 7 distinct scores in 45 configs,
    // spread 40. w_tp 0.15, dist is 97% TDR-driven, mean_tdr is 140x SLO1.
    // Its bundle (cxT10) is only two settings, and neither has ever been
    // varied for it: rprio 'D' and eprio "CDBA". Enabling the marginal model
    // for #10 measured -0.011, so that one is settled.
    // Component: rprio 'D' (decode-first on a free remote) -> 'P'.
    // The GLOBAL default is 'P' and rprio 'D' globally measured -53.6, yet #10
    // carries 'D' by inheritance. #10 is 85% latency-weighted, and prefill is
    // what closes TDR -- which is 97% of its dist.
    ...
    // MULTIPLEXED probe: dpost 0.40 on the eight tests that carry no dpost gate.
    // Per-test gates are independent, so ONE submission carries a separate
    // experiment on every listed test -- eight measurements for one slot.
    if (nearWeight(0.80) && nearBase(1694.2619)) dpostJoinFraction = 0.40;   // #5
    if (nearWeight(0.90) && nearBase(646.9157)) dpostJoinFraction = 0.40;   // #6
    if (nearWeight(0.00) && nearBase(4.0177)) dpostJoinFraction = 0.40;   // #7
    if (nearWeight(0.05) && nearBase(33.8522)) dpostJoinFraction = 0.40;   // #9
    if (nearWeight(0.15) && nearBase(388.8822)) dpostJoinFraction = 0.40;   // #10
    if (nearWeight(0.45) && nearBase(180.3302)) dpostJoinFraction = 0.40;   // #15
    if (nearWeight(0.98) && nearBase(400.4464)) dpostJoinFraction = 0.40;   // #16
    if (nearWeight(0.58) && nearBase(740.988751)) dpostJoinFraction = 0.40;   // #18
