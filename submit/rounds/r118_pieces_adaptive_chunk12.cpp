...
int main() {
    ...
    // pieces = 0 selects the ADAPTIVE split in pieceCountFor: p = dur/(chunk*S),
    // i.e. split a prefill in proportion to how long it actually runs, instead
    // of a fixed count. Every splitting attempt so far has been a FIXED count
    // (pieces 2 lost on corpus2, dsplit measured -18.023 on #6 on the judge),
    // and a fixed count pays extra S even on short prefills that never block
    // anything. The adaptive rule only splits the long ones -- which is the
    // stated purpose of the chunk rule: stop a long serial P PROC from blocking
    // every decoder pinned to that remote. It also makes `chunk` live; with
    // pieces = 1 chunk is never read, which is why A_CHUNK alone was a no-op.
    int pieces = 0;
    if (const char *e = getenv("A_PIECES")) pieces = max(0, atoi(e));
    double chunk = 12.0;   // coarser adaptive split: fewer, larger pieces
