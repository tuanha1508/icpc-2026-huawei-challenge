...
int main() {
    ...
    // The statement says each direction of the link is SERIAL (line 73), and a
    // D PRE / D POST completion queues "one local-to-remote transfer per
    // distinct remote computer" (line 181). So a decode group spread over r
    // remotes costs r transfers per iteration, each paying a fixed latency_ms
    // on that serial link -- while the remotes themselves sit idle 94.6% of
    // the time, so their compute is not the constraint. Restricting how many
    // distinct remotes are used shrinks the per-iteration transfer count on the
    // one resource that is actually saturated. Never judge-tested.
    int ruse = 0;
    if (const char *e = getenv("A_RUSE")) ruse = atoi(e);
    if (ruse <= 0 || ruse > K) ruse = K;
    if (getenv("A_RUSE") == nullptr && ruse > 4) ruse = 4;
