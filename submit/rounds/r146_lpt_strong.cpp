...
int main() {
    ...
    // MAKESPAN vs MEAN FLOW TIME. tp = sum(L_out) / (last_token - first_arrival)
    // depends ONLY on when the LAST token lands, so a throughput-weighted test
    // is a makespan objective -- and the classic rule for minimising makespan
    // is LONGEST-processing-time-first, not shortest. 'S' (SJF) minimises MEAN
    // flow time, which is right for the latency-weighted tests and wrong for
    // these: a short-first scheduler clears easy requests early and strands a
    // long one at the end, which then sets the makespan.
    // #6 is the case in point: w_tp 0.90 with 599 points of throughput headroom,
    // while norm_c 0.9917 against dist_base 646.9 means latency could degrade
    // ~120x before costing the remaining 100. Trading latency for makespan is
    // almost free there.
    // Narrower: only the STRONGLY throughput-dominated tests.
    char order = (w_tp >= 0.75) ? 'L' : ((w_c > 0.0) ? 'S' : 'F');
