...
int main() {
    ...
    // r294 PROBE.  THE biggest untested cell.  #6 carries 584 points of cap and has been
    // running dgfrac 0.25 -- not from a gate but from the declaration itself,
    // `targetTest6 ? 0.25 : 0.18`, which is why it never showed up in a
    // resolve_knob audit.  useMarginal is true for w_tp 0.90, so
    // fixedDecodeWaves is true and the dynamic rule never rewrites it: 0.25
    // is what has actually run all campaign.  #5 gained on the way to 1.00.
    if (nearWeight(0.90) && nearBase(646.9157)) { dgfrac = 1.00; dgfracForced = true; }   // #6

    // SINGLE-CELL PROBE (r279 dgfrac 0.95 on #5)
    if (nearWeight(0.80) && nearBase(1694.2619)) { dgfrac = 1.00; dgfracForced = true; }   // #5
