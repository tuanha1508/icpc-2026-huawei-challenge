...
int main() {
    ...
    // As r119, but applied to the whole w_tp == 0 class rather than #3 alone,
    // so this build minus r119 isolates #7's contribution exactly. #7 is the
    // only other zero-weight test (dist_base 4.017728) and no reconstruction
    // of it exists locally, so the judge is the only way to measure it.
    if (nearWeight(0.00)) balw = 4.0;
