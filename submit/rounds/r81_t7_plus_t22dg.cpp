...
int main() {
    ...
    // r81 -- isolate the dgfrac question r77 left unanswered. r77 applied
    // dgfrac 0.90 to #22 AND #16 together and the pair cost about -2.4, so the
    // split was never measured. #22 is the test where D POST batching won
    // biggest (+36.214) and it still has 43.3 open points, so its D PRE twin is
    // worth one clean, isolated bit. #16 is deliberately excluded.
    if (getenv("A_DGFRAC") == nullptr &&
        nearWeight(0.50) && nearBase(80003.2264)) { dgfrac = 0.90; dgfracForced = true; }
    ...
    if (getenv("A_DPOSTFRAC") == nullptr) {
        ...
        // r80 PROBE -- #7, the best remaining payoff structure of any test.
        // #7 is w_tp = 0.00, i.e. PURE latency, at 248.9 pts per unit dist. Its
        // tpot (65.63) sits just above SLO2 (52.77), so ex_tpot = 0.2437 of a
        // dist of 0.3377. A 19.6% tpot cut drops ex_tpot to ZERO and removes it
        // from dist entirely: score 915.319 -> 941.815, **+26.50**.
        // dpost cut tpot 24.9% on #22 and 37.2% on #16 -- both clear 19.6% --
        // and it left tdr unchanged on #22/#4 (-0.01% on #13), so #7's tdr half
        // should be untouched. #7 is also dense (tdr/tpot = 13), the condition
        // that separated the winners from the losers.
        else if (nearWeight(0.00) && nearBase(4.0177)) dpostJoinFraction = 0.90; // #7
    }
