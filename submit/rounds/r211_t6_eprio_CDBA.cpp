...
int main() {
    ...
    for (;;) {
        ...
        // Component: #6 has NO forced eprio -- it takes whatever the marginal
        // model picks each frame, plus the "CDAB" fallback. Forcing a fixed
        // prefill-first order has never been tried for it. On #13, which also
        // runs a forced string, the decode-order axis alone was worth 23.89.
        if (targetTest6 && !eprioForced) { eprio = "CDBA"; eprioForced = true; }
