...
int main() {
    ...
    // MULTIPLEX: radapt = false on 17 tests. COMBINED: #8 at dpost 0.97 while all 17 others test radapt OFF. The adaptive-concurrency loop steers N on dist; on throughput-weighted tests dist is the minor term, and radapt has never been gated per-test.
    // Independent gates -- one slot, many experiments. Losers cost only
    // their own test and are dropped after the per-test harvest.
    if (nearWeight(0.00) && nearBase(1.156784)) radapt = false;   // #3
    if (nearWeight(0.30) && nearBase(27.1461)) radapt = false;   // #4
    if (nearWeight(0.80) && nearBase(1694.2619)) radapt = false;   // #5
    if (nearWeight(0.90) && nearBase(646.9157)) radapt = false;   // #6
    if (nearWeight(0.00) && nearBase(4.0177)) radapt = false;   // #7
    if (nearWeight(0.05) && nearBase(33.8522)) radapt = false;   // #9
    if (nearWeight(0.15) && nearBase(388.8822)) radapt = false;   // #10
    if (nearWeight(0.99) && nearBase(4.490298)) radapt = false;   // #12
    if (nearWeight(0.75) && nearBase(16.888522)) radapt = false;   // #13
    if (nearWeight(0.65) && nearBase(0.865366)) radapt = false;   // #14
    if (nearWeight(0.45) && nearBase(180.3302)) radapt = false;   // #15
    if (nearWeight(0.98) && nearBase(400.4464)) radapt = false;   // #16
    if (nearWeight(0.67) && nearBase(3259.1504)) radapt = false;   // #17
    if (nearWeight(0.58) && nearBase(740.988751)) radapt = false;   // #18
    if (nearWeight(1.00) && nearBase(41341.873)) radapt = false;   // #19
    if (nearWeight(0.50) && nearBase(2917.9071)) radapt = false;   // #21
    if (nearWeight(0.50) && nearBase(80003.2235)) radapt = false;   // #22
    ...
    if (nearWeight(0.25) && nearBase(10.8848)) dpostJoinFraction = 0.97;   // #8
