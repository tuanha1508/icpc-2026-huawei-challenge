...
int main() {
    ...
    // MULTIPLEXED per-test probe. Gates are independent, so one
    // submission carries eight experiments instead of one. Extreme values
    // are used deliberately: the largest gain in this project's history
    // was #22 dpost 0.90 = +36.2, a value that is catastrophic globally
    // (-27 on #17, -6 on #13) and can only ever be found by gating.
    if (nearWeight(0.00) && nearBase(1.156784)) dgfrac = 0.60;   // #3
    if (nearWeight(0.80) && nearBase(1694.261926)) dgfrac = 0.05;   // #5
    if (nearWeight(0.90) && nearBase(646.913085)) dgfrac = 0.05;   // #6
    if (nearWeight(0.15) && nearBase(388.882182)) dgfrac = 0.60;   // #10
    if (nearWeight(0.75) && nearBase(16.888562)) dgfrac = 0.05;   // #13

    // dgfrac 0.32 lost 13.60 globally but WON on two tests: #12 +0.24 and
    // #18 +0.03. Gate it to exactly those two.
    if ((nearWeight(0.99) && nearBase(4.490298)) ||
        (nearWeight(0.58) && nearBase(740.988751))) dgfrac = 0.32;
    ...
    if (nearWeight(0.99) && nearBase(4.490301)) dpostJoinFraction = 0.005;   // #12
    if (nearWeight(0.65) && nearBase(0.865366)) dpostJoinFraction = 0.005;   // #14
    ...
    if (nearWeight(0.67) && nearBase(3259.144289)) maxg = 4096;   // #17
