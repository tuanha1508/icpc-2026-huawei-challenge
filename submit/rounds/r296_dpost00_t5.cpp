...
int main() {
    ...
    if (nearWeight(0.80) && nearBase(1694.2619)) dpostJoinFraction = 0.00;   // #5
    // r296 PROBE.  r291 took #5's dpostJoinFraction from 0.25 to 1.00 and lost 42.19, so the
    // D POST hold wants the OPPOSITE of the D PRE hold on the same test:
    // dgfrac 1.00 gained, dpost 1.00 lost hard.  That is coherent -- hold D PRE
    // to assemble one maximal wave, then flush D POST the moment tokens land so
    // the members go ready again instead of queueing behind a join.  Walking to
