...
int main() {
    ...
    if (getenv("A_RPORDER") == nullptr && nearWeight(0.00) && nearBase(4.017728))
        rporder = 'S';
    // #12 is PREFILL-bound, not decode-bound: removing its decode-group cap
    // entirely (r147) was an exact no-op, so groups never even reach 8, and
    // mean_tdr is 1,253,095 -- requests wait over a million ms for a first
    // token. Throughput there is set by how fast requests get THROUGH prefill.
    // Ordering a remote's prefill queue shortest-first maximises the number of
    // requests that reach decode early, which is exactly what raises tp. #12
    // has by far the deepest queues of any test, so reordering has the most
    // room to matter there -- it binds only 2/60 globally because most tests
    // never have more than one prefill queued.
    if (getenv("A_RPORDER") == nullptr && nearWeight(0.99) && nearBase(4.490301))
        rporder = 'S';
