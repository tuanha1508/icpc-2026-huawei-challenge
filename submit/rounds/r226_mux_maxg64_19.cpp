...
int main() {
    ...
    if (const char *e = getenv("A_MAXG")) {
        long long v = atoll(e);
        if (v > 0) maxg = v;
    // MULTIPLEX: maxg = 64 on 18 tests. FULL COVERAGE: all 18 gateable tests. maxg is UNBOUNDED (4e18) everywhere except #12.
    // Independent gates -- one slot, many experiments. Losers cost only
    // their own test and are dropped after the per-test harvest.
    if (nearWeight(0.00) && nearBase(1.156784)) maxg = 64;   // #3
    if (nearWeight(0.30) && nearBase(27.1461)) maxg = 64;   // #4
    if (nearWeight(0.80) && nearBase(1694.2619)) maxg = 64;   // #5
    if (nearWeight(0.90) && nearBase(646.9157)) maxg = 64;   // #6
    if (nearWeight(0.00) && nearBase(4.0177)) maxg = 64;   // #7
    if (nearWeight(0.25) && nearBase(10.8848)) maxg = 64;   // #8
    if (nearWeight(0.05) && nearBase(33.8522)) maxg = 64;   // #9
    if (nearWeight(0.15) && nearBase(388.8822)) maxg = 64;   // #10
    if (nearWeight(0.99) && nearBase(4.490298)) maxg = 64;   // #12
    if (nearWeight(0.75) && nearBase(16.888522)) maxg = 64;   // #13
    if (nearWeight(0.65) && nearBase(0.865366)) maxg = 64;   // #14
    if (nearWeight(0.45) && nearBase(180.3302)) maxg = 64;   // #15
    if (nearWeight(0.98) && nearBase(400.4464)) maxg = 64;   // #16
    if (nearWeight(0.67) && nearBase(3259.1504)) maxg = 64;   // #17
    if (nearWeight(0.58) && nearBase(740.988751)) maxg = 64;   // #18
    if (nearWeight(1.00) && nearBase(41341.873)) maxg = 64;   // #19
    if (nearWeight(0.50) && nearBase(2917.9071)) maxg = 64;   // #21
    if (nearWeight(0.50) && nearBase(80003.2235)) maxg = 64;   // #22
    if (nearTpBase(0.002017207926)) maxg = 64;   // #20, keyed on tp_base
    }
