...
int main() {
    ...
    // MULTIPLEX: dgfrac = 0.08 on 7 tests. Low side of the same seven.
    // Independent gates -- one slot, many experiments. Losers cost only
    // their own test and are dropped after the per-test harvest.
    if (nearWeight(0.00) && nearBase(1.156784)) dgfrac = 0.08;   // #3
    if (nearWeight(0.05) && nearBase(33.8522)) dgfrac = 0.08;   // #9
    if (nearWeight(0.15) && nearBase(388.8822)) dgfrac = 0.08;   // #10
    if (nearWeight(0.65) && nearBase(0.865366)) dgfrac = 0.08;   // #14
    if (nearWeight(0.45) && nearBase(180.3302)) dgfrac = 0.08;   // #15
    if (nearWeight(1.00) && nearBase(41341.873)) dgfrac = 0.08;   // #19
    if (nearWeight(0.50) && nearBase(80003.2235)) dgfrac = 0.08;   // #22
