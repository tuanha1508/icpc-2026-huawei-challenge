...
int main() {
    ...
    // MULTIPLEX: dpostJoinFraction = 0.15 on 15 tests. Same fifteen at 0.15 -- the value most of them already inherit, but several carry a DIFFERENT gated value (#4 0.30, #8 0.25, #12/#17 0.08, #13 0.15, #21 0.005), so this is a real change for those.
    // Independent gates -- one slot, many experiments. Losers cost only
    // their own test and are dropped after the per-test harvest.
    if (nearWeight(0.00) && nearBase(1.156784)) dpostJoinFraction = 0.15;   // #3
    if (nearWeight(0.30) && nearBase(27.1461)) dpostJoinFraction = 0.15;   // #4
    if (nearWeight(0.80) && nearBase(1694.2619)) dpostJoinFraction = 0.15;   // #5
    if (nearWeight(0.25) && nearBase(10.8848)) dpostJoinFraction = 0.15;   // #8
    if (nearWeight(0.05) && nearBase(33.8522)) dpostJoinFraction = 0.15;   // #9
    if (nearWeight(0.15) && nearBase(388.8822)) dpostJoinFraction = 0.15;   // #10
    if (nearWeight(0.99) && nearBase(4.490298)) dpostJoinFraction = 0.15;   // #12
    if (nearWeight(0.75) && nearBase(16.888522)) dpostJoinFraction = 0.15;   // #13
    if (nearWeight(0.65) && nearBase(0.865366)) dpostJoinFraction = 0.15;   // #14
    if (nearWeight(0.45) && nearBase(180.3302)) dpostJoinFraction = 0.15;   // #15
    if (nearWeight(0.98) && nearBase(400.4464)) dpostJoinFraction = 0.15;   // #16
    if (nearWeight(0.67) && nearBase(3259.1504)) dpostJoinFraction = 0.15;   // #17
    if (nearWeight(0.58) && nearBase(740.988751)) dpostJoinFraction = 0.15;   // #18
    if (nearWeight(1.00) && nearBase(41341.873)) dpostJoinFraction = 0.15;   // #19
    if (nearWeight(0.50) && nearBase(2917.9071)) dpostJoinFraction = 0.15;   // #21
