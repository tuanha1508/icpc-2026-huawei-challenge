...
int main() {
    ...
    // #12: `order` is one of the few levers KNOWN to bind there -- LPT ('L')
    // measured -1.88 on #12 specifically. The third branch,
    // key = (t - arrivalT)/svcEst, is a highest-response-ratio rule that ages
    // waiting requests and is unreachable by default (any letter that is not
    // F/L/S selects it). With mean_tdr 1,253,095, #12 has by far the worst
    // waiting in the set, which is exactly the condition HRRN is designed for.
    char order = (fabs(w_tp - 0.99) < 1e-9
