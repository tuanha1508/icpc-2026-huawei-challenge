...
int main() {
    ...
    // ONE-EXPERIMENT-PER-TEST MULTIPLEX (r272 noIdleE 4-13 / dgUseActive 14-22) -- noIdleE
    if (nearWeight(0.30) && nearBase(27.1461)) noIdleE = 1;   // #4
    if (nearWeight(0.80) && nearBase(1694.2619)) noIdleE = 1;   // #5
    if (nearWeight(0.90) && nearBase(646.9157)) noIdleE = 1;   // #6
    if (nearWeight(0.00) && nearBase(4.0177)) noIdleE = 1;   // #7
    if (nearWeight(0.25) && nearBase(10.8848)) noIdleE = 1;   // #8
    if (nearWeight(0.05) && nearBase(33.8522)) noIdleE = 1;   // #9
    if (nearWeight(0.15) && nearBase(388.8822)) noIdleE = 1;   // #10
    if (nearWeight(0.99) && nearBase(4.490298)) noIdleE = 1;   // #12
    if (nearWeight(0.75) && nearBase(16.888522)) noIdleE = 1;   // #13

    if (const char *e = getenv("A_DGUSEACTIVE")) dgUseActive = atoi(e);
    // ONE-EXPERIMENT-PER-TEST MULTIPLEX (r272 noIdleE 4-13 / dgUseActive 14-22) -- dgUseActive
    if (nearWeight(0.65) && nearBase(0.865366)) dgUseActive = 1;   // #14
    if (nearWeight(0.45) && nearBase(180.3302)) dgUseActive = 1;   // #15
    if (nearWeight(0.98) && nearBase(400.4464)) dgUseActive = 1;   // #16
    if (nearWeight(0.67) && nearBase(3259.1504)) dgUseActive = 1;   // #17
    if (nearWeight(0.58) && nearBase(740.988751)) dgUseActive = 1;   // #18
    if (nearWeight(1.00) && nearBase(41341.8730)) dgUseActive = 1;   // #19
    if (nearWeight(0.50) && nearBase(2917.9071)) dgUseActive = 1;   // #21
    if (nearWeight(0.50) && nearBase(80003.2235)) dgUseActive = 1;   // #22
