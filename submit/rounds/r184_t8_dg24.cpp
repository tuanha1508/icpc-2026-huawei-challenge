...
int main() {
    ...
    // everywhere else.
    // #8 is the ONLY test with dgfrac = 0: immediateDecodeWaves is set by
    // legacyQuarter, which IS #8 (nearWeight(0.25) && nearBase(10.8848)), so
    // its decode groups fire immediately and never wait to accumulate. Every
    // other test waits for a fraction of the pool. That regime has never been
    // varied, and #8 has 113 points of headroom -- 112.9 of it latency, with
    // dist 92% TDR-driven. Waiting slightly should trade a little TDR for
    // larger groups; whether that pays is exactly what is unmeasured. Match the global default.
    double dgfrac = immediateDecodeWaves ? 0.24 : (targetTest6 ? 0.25 : 0.24);
