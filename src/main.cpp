...
int main() {
    ...
    bool rpsjf = true;                       // per-remote prefill SJF
    ...
    for (;;) {
        ...
        for (int k = 0; k < K; ++k) {
            ...
            if (!bProcRdy[k].empty()) {
                ...
                // SJF on the REMOTE's prefill queue, not just on admission.
                // Shortest prefill_proc first minimises mean completion on that
                // remote, which lowers TDR directly, and it clears short
                // prefills out of the way so decode is blocked less, which
                // lowers TPOT. Admission-side SJF cannot do this: once requests
                // are pinned, the per-remote service order is a separate
                // decision that was still FIFO.
                if (rpsjf) {
