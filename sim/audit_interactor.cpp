// Work-conservation audit build of fast_interactor.
// Decomposes E idle time into:
//   idle_avoid   - an E-runnable request existed (ARRIVED / POST_RDY / DEC_RDY / DPOST_RDY)
//   idle_forced  - nothing E-runnable existed (waiting on remote round-trips)
// Also breaks E busy time down by action family and setup overhead S.
// Usage: audit_interactor <test.txt> <solver-binary>
#include <bits/stdc++.h>
#include <unistd.h>
#include <sys/wait.h>
using namespace std;
...
struct Req {
    ...
    bool seen=false; // has the ARR event fired yet
};

struct Ev {
    double t; int ord; long long seq; int kind;      // 0 ARR,1 TDN,2 XDN
    int rid=0;
    string spec; double dur=0;
    function<vector<string>(double)> eff;
    bool up=false; int remote=0; long long size=0; string xk; vector<int> rids;
};
...
struct Sim {
    ...
    double perTok=0;
    // audit accumulators
    double eBusy[4]={0,0,0,0};   // P PRE, P POST, D PRE, D POST (incl S)
    double eSetup=0;             // S share of E busy
    long long eTasks[4]={0,0,0,0};
    long long eTokens[4]={0,0,0,0}; // batch sizes summed for D PRE/D POST
...
int main(int argc,char**argv){
    ...
    // idle accounting
    double idleAvoid=0, idleForced=0, idleAvoidPreOnly=0;
    double idleAvoidDec=0, idleAvoidPost=0, idleAvoidDpost=0;
    double prevT=-1; bool prevIdle=false, prevRunnable=false, prevPreOnly=false;
    bool prevHadDec=false, prevHadPost=false, prevHadDpost=false;
    double firstDecision=-1, lastEvent=0;

    while(!s.heap.empty()){
        double t=s.heap.top().t;
        // close previous idle interval [prevT, t)
        if(prevT>=0 && t>prevT && prevIdle){
            double dt=t-prevT;
            if(prevRunnable){
                idleAvoid+=dt;
                if(prevPreOnly) idleAvoidPreOnly+=dt;
                if(prevHadDec)  idleAvoidDec+=dt;
                if(prevHadPost) idleAvoidPost+=dt;
                if(prevHadDpost)idleAvoidDpost+=dt;
            } else idleForced+=dt;
        }
        lastEvent=t;
        vector<Ev> batch;
        ...
        vector<string> lines;
        for(auto&e:batch){
            if(e.kind==0){ Req&r=s.reqs[e.rid]; r.seen=true;
        ...
        for(int i=0;i<n;++i){
            ...
            if(fam=="P"){
                if(step=="PRE"){ string a,b; readTok(a); readTok(b);
                    ...
                    s.eBusy[0]+=s.S+dur; s.eSetup+=s.S; s.eTasks[0]++; s.eTokens[0]++;
                    ...
                    s.eBusy[1]+=s.S+dur; s.eSetup+=s.S; s.eTasks[1]++; s.eTokens[1]++;
                ...
                if(step=="PRE"){ for(int id:rids) s.reqs[id].stage=DPRE_RUN;
                    ...
                    s.eBusy[2]+=s.S+dur; s.eSetup+=s.S; s.eTasks[2]++; s.eTokens[2]+=m;
                    ...
                    s.eBusy[3]+=s.S+dur; s.eSetup+=s.S; s.eTasks[3]++; s.eTokens[3]+=m;
        ...
        // snapshot post-decision state for the interval starting at t
        prevT=t; prevIdle=!s.busyE;
        prevRunnable=false; prevPreOnly=false;
        prevHadDec=prevHadPost=prevHadDpost=false;
        if(prevIdle){
            bool anyPre=false;
            for(auto&r:s.reqs){
                if(!r.seen) continue;
                if(r.stage==ARRIVED) anyPre=true;
                else if(r.stage==POST_RDY) prevHadPost=true;
                else if(r.stage==DEC_RDY)  prevHadDec=true;
                else if(r.stage==DPOST_RDY)prevHadDpost=true;
            }
            prevRunnable = anyPre||prevHadPost||prevHadDec||prevHadDpost;
            prevPreOnly  = anyPre && !prevHadPost && !prevHadDec && !prevHadDpost;
        }
        if(firstDecision<0) firstDecision=t;
    }
    fprintf(w,"END\n"); fflush(w); fclose(w); fclose(rd);
    int st; waitpid(pid,&st,0);

    long long totalTok=0; double firstArr=1e300,lastTok=-1e300,tdrSum=0;
    long long gaps=0; double span=0;
    for(auto&r:s.reqs){ totalTok+=r.lout; firstArr=min(firstArr,r.arrival);
        lastTok=max(lastTok,r.lastTok); tdrSum+=r.tdr_done-r.arrival;
        gaps+=r.lout-1; if(r.lout>1) span+=r.lastTok-r.firstTok; }
    double elapsed=lastTok-firstArr;
    double tp = elapsed>0 ? totalTok/elapsed : 1e300;
    double tdr = tdrSum/(double)s.reqs.size();
    ...
    double eBusyTot=s.eBusy[0]+s.eBusy[1]+s.eBusy[2]+s.eBusy[3];
    printf("--- E audit (span %.3f) ---\n", elapsed);
    printf("E busy    %.3f (%.2f%%)  [P_PRE %.1f | P_POST %.1f | D_PRE %.1f | D_POST %.1f | setup_S %.1f]\n",
        eBusyTot, 100*eBusyTot/elapsed, s.eBusy[0],s.eBusy[1],s.eBusy[2],s.eBusy[3],s.eSetup);
    printf("idle_forced %.3f (%.2f%%)\n", idleForced, 100*idleForced/elapsed);
    printf("idle_avoid  %.3f (%.2f%%)  [preOnly %.1f | hadDEC %.1f | hadPPOST %.1f | hadDPOST %.1f]\n",
        idleAvoid, 100*idleAvoid/elapsed, idleAvoidPreOnly, idleAvoidDec, idleAvoidPost, idleAvoidDpost);
    printf("tasks: PPRE %lld PPOST %lld DPRE %lld DPOST %lld | avg batch DPRE %.2f DPOST %.2f\n",
        s.eTasks[0],s.eTasks[1],s.eTasks[2],s.eTasks[3],
        s.eTasks[2]? (double)s.eTokens[2]/s.eTasks[2]:0.0,
        s.eTasks[3]? (double)s.eTokens[3]/s.eTasks[3]:0.0);
    return 0;
}
